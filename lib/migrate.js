...
// Deterministic UUID v4-shaped from a source ID + namespace so the same
// wknd-stats entity always maps to the same portal UUID across re-migrations.
const NAMESPACE = 'wknd-portal-v1';
function stableUUID(sourceId) {
  const hex = createHash('sha256').update(`${NAMESPACE}:${sourceId}`).digest('hex');
...
portal.exec(`
  ...
  CREATE TABLE IF NOT EXISTS player_totals (
    ...
    fg3m INTEGER NOT NULL DEFAULT 0, fg4m INTEGER NOT NULL DEFAULT 0,
    fg2m_miss INTEGER NOT NULL DEFAULT 0,
    fg3m_miss INTEGER NOT NULL DEFAULT 0, fg4m_miss INTEGER NOT NULL DEFAULT 0,
...
srcTeams.forEach(t   => { teamMap[t.id]   = stableUUID(`team:${t.id}`); });
srcPlayers.forEach(p => { playerMap[p.id] = stableUUID(`player:${p.id}`); });
srcGames.forEach(g   => { gameMap[g.id]   = stableUUID(`game:${g.id}`); });
...
for (const t of srcTotals) {
  ...
  stmtTotals.run(newId, t.games_played, t.pts, t.ast, t.reb, t.stl, t.blk,
    t.turnover, t.pf, t.fg2m, t.fg3m, t.fg4m || 0, t.fg2m_miss, t.fg3m_miss, t.fg4m_miss || 0, t.ftm, t.ft_miss);
  totalsCount++;
}
...
for (const s of srcStats) {
  ...
  stmtStats.run(
    ...
    s.fg2m, s.fg3m, s.fg4m || 0, s.fg2m_miss, s.fg3m_miss, s.fg4m_miss || 0, s.ftm, s.ft_miss,
    s.minutes || ''
  );
  statsCount++;
}
