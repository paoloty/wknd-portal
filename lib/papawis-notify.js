...
// Fires once, right when an admin marks a game complete — same "who to email" list as
// cancellation (real signups only, guest_name = ''), but the amount is a per-player total:
// a sponsor who also brought a guest gets charged once per slot in the complete route
// (lib/portal-db.js recordTransaction, called once per signup row), so their receipt sums
// both rather than just quoting the flat per-player price.
export async function sendPapawisCompletionEmails(gameId, pricePerPlayer) {
  ...
  for (const s of allConfirmed) totalsByPlayer[s.player_id] = (totalsByPlayer[s.player_id] || 0) + price;
  ...
  const errors = [];
  for (const signup of recipients) {
    const amount = totalsByPlayer[signup.player_id];
