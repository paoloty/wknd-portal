...
import {
  ...
  papawisCompletedEmail, papawisUpdatedEmail,
} from './mailer.js';
...
// Fires when an admin edits location/time on a still-open game — same recipient list as
// cancellation (everyone confirmed or waitlisted with an email on file). changeLines is a
// small array of human-readable strings built by the caller (e.g. "New location: ...",
// "New time: ..."), so this stays agnostic to which fields actually moved.
export async function sendPapawisUpdateEmails(gameId, changeLines) {
  const game = getPapawisGame(gameId);
  if (!game || !changeLines?.length) return { sent: 0, errors: [] };
  const recipients = getPapawisSignupsWithEmail(gameId);
  const line = gameLine(game);
  let sent = 0;
  const errors = [];
  for (const signup of recipients) {
    ...
    try {
      await sendMail({
        to: signup.player_email,
        ...papawisUpdatedEmail({ name: signup.player_name, gameTitle: game.title || 'Papawis', gameLine: line, changeLines, url: gameUrl(gameId) }),
      });
      logPapawisActivity({ gameId, eventType: 'updated_notified', playerId: signup.player_id, playerName: signup.player_name, notes: changeLines.join('; ') });
      createNotification({
        playerId: signup.player_id,
        type: 'papawis_updated',
        title: `${game.title || 'Papawis'} details changed`,
        body: changeLines.join(' · '),
        link: `/papawis#pw-game-${gameId}`,
      });
      sent++;
    } catch (e) {
      errors.push({ signupId: signup.id, error: e.message });
