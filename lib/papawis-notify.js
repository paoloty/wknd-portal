...
function nextGameLines(afterDate, excludeGameId) {
  ...
  if (!next) return { nextGameLine: '', nextGameOpensLine: '' };
  ...
  for (const signup of due) {
    ...
    try {
      ...
      if (signup.status === 'waitlist') {
        const { nextGameLine, nextGameOpensLine } = nextGameLines(signup.game_date, signup.game_id);
        ...
        email = papawisWaitlistEmail({
          ...
          waitlistPosition: waitlistPosition || null, nextGameLine, nextGameOpensLine,
        });
...
export async function sendPapawisCancellationEmails(gameId) {
  ...
  const { nextGameLine, nextGameOpensLine } = nextGameLines(game.date, gameId);
  ...
  for (const signup of recipients) {
    ...
    try {
      await sendMail({
        ...
        ...papawisCancelledEmail({ name: signup.player_name, gameTitle: game.title || 'Papawis', gameLine: line, nextGameLine, nextGameOpensLine }),
      });
