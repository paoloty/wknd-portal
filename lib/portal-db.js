...
const stmtInsertMarketplaceCommitment = db.prepare(`
  INSERT INTO marketplace_commitments (id, listing_id, player_id, variant, status, committed_at)
  VALUES (?, ?, ?, ?, 'committed', ?)
`);
export function commitToMarketplaceListing(listingId, playerId, variant = '') {
  const id = crypto.randomUUID();
  stmtInsertMarketplaceCommitment.run(id, listingId, playerId, variant, Date.now());
  return id;
}
