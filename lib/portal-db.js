...
`);
{
  ...
  // Group 00 (Liveness Check) — set at final submission if the registrant used the "skip"
  // escape hatch instead of capturing a photo (no camera access, hardware issue, etc.).
  // The actual capture, when one exists, lives in liveness_captures — this column is only
  // the "did they explicitly decline" signal for admin, purely discretionary like every
  // other flag in this flow, never a hard gate on submission.
  if (!_ssCols.includes('liveness_skipped_at')) db.exec(`ALTER TABLE season_signups ADD COLUMN liveness_skipped_at INTEGER NOT NULL DEFAULT 0`);
  // Head-collected jersey number for a signup with no live player_id yet — see /my-team.
  // Distinct from registrations.jersey_pref (a loose self-reported preference asked once at
  // initial registration, never checked against teammates); this one is scoped to the team
  // and validated for uniqueness at the time a head enters it.
  if (!_ssCols.includes('jersey_number'))   db.exec(`ALTER TABLE season_signups ADD COLUMN jersey_number TEXT NOT NULL DEFAULT ''`);
  // Admin-triggered "please fill in your jersey details" flow (see /jersey-request) — a
  // second path to the same jersey_number/jersey_top/jersey_shorts/pockets columns above,
  // this time filled in by the player themselves via a tokenized emailed link rather than
  // their head. jersey_name is new here (not asked at season-signup): the name printed on
  // the jersey, which may differ from their registered name.
  if (!_ssCols.includes('jersey_name'))              db.exec(`ALTER TABLE season_signups ADD COLUMN jersey_name              TEXT    NOT NULL DEFAULT ''`);
  // Free-text, only meaningful once shorts are actually selected (e.g. "run long, prefer
  // one size down") — optional even then, never validated or required.
  if (!_ssCols.includes('jersey_shorts_notes'))      db.exec(`ALTER TABLE season_signups ADD COLUMN jersey_shorts_notes      TEXT    NOT NULL DEFAULT ''`);
  if (!_ssCols.includes('jersey_request_token'))     db.exec(`ALTER TABLE season_signups ADD COLUMN jersey_request_token     TEXT    NOT NULL DEFAULT ''`);
  if (!_ssCols.includes('jersey_request_token_exp')) db.exec(`ALTER TABLE season_signups ADD COLUMN jersey_request_token_exp INTEGER NOT NULL DEFAULT 0`);
  if (!_ssCols.includes('jersey_requested_at'))      db.exec(`ALTER TABLE season_signups ADD COLUMN jersey_requested_at      INTEGER NOT NULL DEFAULT 0`);
  if (!_ssCols.includes('jersey_submitted_at'))      db.exec(`ALTER TABLE season_signups ADD COLUMN jersey_submitted_at      INTEGER NOT NULL DEFAULT 0`);
  // Start Season review screen (/admin/season/teams/review) — per-player charge overrides
  // an admin sets before actually charging anyone. Excluding top/shorts only skips billing
  // for that item this cycle; jersey_top/jersey_shorts themselves are untouched, so the size
  // stays on file. extra_charge_amount can be negative (a discount) or positive (an add-on);
  // extra_charge_label is what shows next to it both on the review screen and in the
  // confirmation email.
  if (!_ssCols.includes('jersey_top_excluded'))      db.exec(`ALTER TABLE season_signups ADD COLUMN jersey_top_excluded      INTEGER NOT NULL DEFAULT 0`);
  if (!_ssCols.includes('jersey_shorts_excluded'))   db.exec(`ALTER TABLE season_signups ADD COLUMN jersey_shorts_excluded   INTEGER NOT NULL DEFAULT 0`);
  if (!_ssCols.includes('extra_charge_amount'))      db.exec(`ALTER TABLE season_signups ADD COLUMN extra_charge_amount      REAL    NOT NULL DEFAULT 0`);
  if (!_ssCols.includes('extra_charge_label'))       db.exec(`ALTER TABLE season_signups ADD COLUMN extra_charge_label       TEXT    NOT NULL DEFAULT ''`);
}

// ── Liveness Check (season-signup Group 00) ─────────────────────────────────────
// Admin-reference-only photo capture, explicitly NOT identity verification — no automated
// matching against players.picture_url, purely a side-by-side glance for whoever reviews
// the waitlist (see the admin comparison view). Retention: most-recent-only per
// registration, enforced by UNIQUE(reg_id) + an upsert — a new capture silently replaces
// whatever was there before rather than accumulating history, per the confirmed default.
// "Nothing goes out" is enforced by never exposing photo_data through any public route —
// only an admin-authed byte-serving endpoint reads it (see server.js).
db.exec(`
  CREATE TABLE IF NOT EXISTS liveness_captures (
    ...
    via         TEXT NOT NULL DEFAULT 'inline',
    consent_at  INTEGER NOT NULL,
    captured_at INTEGER NOT NULL,
    UNIQUE(reg_id)
  )
`);
// The playful "grab something and hold it up" line shown during capture (see
// LIVENESS_PROMPTS in server.js) — purely for context when admin reviews the photo later,
// never validated against what's actually in the picture.
if (!db.prepare('PRAGMA table_info(liveness_captures)').all().some(c => c.name === 'prompt')) {
  db.exec(`ALTER TABLE liveness_captures ADD COLUMN prompt TEXT NOT NULL DEFAULT ''`);
}
const stmtUpsertLivenessCapture = db.prepare(`
  INSERT INTO liveness_captures (id, reg_id, player_id, season, photo_data, via, prompt, consent_at, captured_at)
  VALUES (@id, @regId, @playerId, @season, @photoData, @via, @prompt, @consentAt, @capturedAt)
  ON CONFLICT(reg_id) DO UPDATE SET
    player_id = excluded.player_id, season = excluded.season, photo_data = excluded.photo_data,
    via = excluded.via, prompt = excluded.prompt, consent_at = excluded.consent_at, captured_at = excluded.captured_at
`);
export function upsertLivenessCapture({ regId, playerId = '', season, photoData, via = 'inline', prompt = '' }) {
  const id = `lv_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  const now = Date.now();
  stmtUpsertLivenessCapture.run({ id, regId, playerId, season, photoData, via, prompt, consentAt: now, capturedAt: now });
  return getLivenessCaptureByRegId(regId);
}
const stmtLivenessCaptureByRegId = db.prepare('SELECT * FROM liveness_captures WHERE reg_id = ?');
export function getLivenessCaptureByRegId(regId) { return stmtLivenessCaptureByRegId.get(regId) || null; }
const stmtLivenessCaptureById = db.prepare('SELECT * FROM liveness_captures WHERE id = ?');
export function getLivenessCaptureById(id) { return stmtLivenessCaptureById.get(id) || null; }
const stmtAllLivenessCaptures = db.prepare('SELECT * FROM liveness_captures ORDER BY captured_at DESC');
export function getAllLivenessCaptures() { return stmtAllLivenessCaptures.all(); }
const stmtDeleteLivenessCapture = db.prepare('DELETE FROM liveness_captures WHERE id = ?');
export function deleteLivenessCapture(id) { stmtDeleteLivenessCapture.run(id); }
...
const stmtInsertSeasonSignup = db.prepare(`
  INSERT OR IGNORE INTO season_signups
    (id, reg_id, season, status, has_balance, balance_amt, notes,
     ...
     waiver_reconfirmed_at, liveness_skipped_at, created_at)
  VALUES (?, ?, ?, 'waitlisted', ?, ?, '', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
// has_balance/balance_amt are overridden below with a live join against player_financials
// instead of trusting season_signups' own has_balance/balance_amt columns — those are only
// ever written once, at signup submission (see insertSeasonSignup's INSERT OR IGNORE), and
// never updated again. Left stale, they'd keep showing an old balance warning forever even
// after the player pays it off. The stored columns still exist (harmless dead weight) since
// nothing else depends on removing them; these queries just no longer read them.
const stmtGetSeasonSignup         = db.prepare(`
  SELECT ss.*, r.full_name, r.email, r.phone, r.player_id,
         ...
         COALESCE(pf.current_balance, 0) AS balance_amt,
         CASE WHEN COALESCE(pf.current_balance, 0) > 0 THEN 1 ELSE 0 END AS has_balance
  FROM season_signups ss
  JOIN registrations r ON ss.reg_id = r.id
  LEFT JOIN players p ON p.id = r.player_id
  LEFT JOIN teams t ON t.id = p.team_id
  LEFT JOIN player_financials pf ON pf.player_id = r.player_id
...
// Players actually rostered for a season — confirmed signup, not waitlisted/withdrawn/rejected.
// Used to scope the ledger overview to "who's here this season" instead of every player
// who has ever existed in the system (past-season players who didn't re-register).
const stmtGetConfirmedSeasonPlayerIds = db.prepare(`
  SELECT DISTINCT r.player_id
  FROM season_signups ss
  JOIN registrations r ON r.id = ss.reg_id
  WHERE ss.season = ? AND ss.status = 'confirmed' AND r.player_id IS NOT NULL AND r.player_id != ''
`);
export function getConfirmedSeasonPlayerIds(season) {
  return new Set(stmtGetConfirmedSeasonPlayerIds.all(String(season)).map(r => r.player_id));
}
...
const stmtGetSeasonRoster   = db.prepare(`
  SELECT sr.*, ss.id AS signup_pk, ss.status, ss.reg_id, ss.jersey_top, ss.jersey_shorts, ss.jersey_number, ss.jersey_name, ss.pockets,
         ss.jersey_shorts_notes, ss.jersey_requested_at, ss.jersey_submitted_at, r.full_name, r.email, r.positions, r.height, r.player_id
...
`);
const stmtSetSeasonSignupJerseyNumber = db.prepare('UPDATE season_signups SET jersey_number = ? WHERE id = ?');
export function setSeasonSignupJerseyNumber(signupId, number) { stmtSetSeasonSignupJerseyNumber.run(String(number), signupId); }

// ── Jersey-request flow (admin-triggered player self-service) ──────────────────────
const stmtSetJerseyRequestToken = db.prepare(`
  UPDATE season_signups SET jersey_request_token = ?, jersey_request_token_exp = ?, jersey_requested_at = ? WHERE id = ?
`);
export function setJerseyRequestToken(signupId, token, expMs) {
  stmtSetJerseyRequestToken.run(token, expMs, Date.now(), signupId);
}
const stmtGetSeasonSignupByJerseyToken = db.prepare(`
  SELECT ss.*, r.full_name, r.email FROM season_signups ss
  JOIN registrations r ON r.id = ss.reg_id
  WHERE ss.jersey_request_token = ? AND ss.jersey_request_token_exp > ?
`);
export function getSeasonSignupByJerseyToken(token) {
  return stmtGetSeasonSignupByJerseyToken.get(token, Date.now()) || null;
}
const stmtSubmitJerseyDetails = db.prepare(`
  UPDATE season_signups
  SET jersey_number = ?, jersey_top = ?, jersey_shorts = ?, pockets = ?, jersey_name = ?, jersey_shorts_notes = ?, jersey_submitted_at = ?
  WHERE id = ?
`);
export function submitJerseyDetails(signupId, { number = '', top = '', shorts = '', pockets = false, jerseyName = '', shortsNotes = '' } = {}) {
  // Notes only ever mean anything alongside an actual shorts size — dropped otherwise so a
  // note doesn't linger and mislead once someone switches back to "None".
  stmtSubmitJerseyDetails.run(String(number), top, shorts, pockets ? 1 : 0, jerseyName, shorts ? shortsNotes : '', Date.now(), signupId);
}
const stmtSetShorts = db.prepare(`
  UPDATE season_signups SET jersey_shorts = ?, pockets = ?, jersey_shorts_notes = ?, jersey_submitted_at = ? WHERE id = ?
`);
// Admin-only quick toggle — the player-facing size picker defaults to "None" and players
// kept missing it or picking it by mistake, so this lets admin add or remove shorts in one
// click without reopening the full jersey-details edit (which also demands name/number).
// Touches only the shorts-related columns; jersey_number/top/name are left exactly as-is.
export function setSeasonSignupShorts(signupId, { shorts = '', pockets = false, shortsNotes = '' } = {}) {
  stmtSetShorts.run(shorts, pockets ? 1 : 0, shorts ? shortsNotes : '', Date.now(), signupId);
}
const stmtSetChargeAdjustments = db.prepare(`
  UPDATE season_signups
  SET jersey_top_excluded = ?, jersey_shorts_excluded = ?, extra_charge_amount = ?, extra_charge_label = ?
  WHERE id = ?
`);
export function setSeasonSignupChargeAdjustments(signupId, { topExcluded = false, shortsExcluded = false, extraAmount = 0, extraLabel = '' } = {}) {
  stmtSetChargeAdjustments.run(topExcluded ? 1 : 0, shortsExcluded ? 1 : 0, Number(extraAmount) || 0, extraAmount ? extraLabel : '', signupId);
}
...
const stmtDeleteRosterSlot  = db.prepare('DELETE FROM season_roster WHERE signup_id = ?');
// A player who's no longer confirmed for this season (withdrew, got rejected) shouldn't
// keep occupying a team slot they were dragged into while still confirmed — called
// wherever a signup's status moves away from 'confirmed'.
export function removeFromSeasonRoster(signupId) { stmtDeleteRosterSlot.run(signupId); }
...
export function insertSeasonSignup(
  regId, season, hasBalance, balanceAmt, jerseyTop = '', jerseyShorts = '', quotaAck = 0, teamPref = '',
  {
    ...
    contactChangedAt = 0, contactChangeNote = '', waiverReconfirmedAt = 0, livenessSkippedAt = 0,
  } = {}
) {
  ...
  stmtInsertSeasonSignup.run(
    ...
    waiverReconfirmedAt || 0, livenessSkippedAt || 0, Date.now()
  );
...
// Super-admin override for a signup's Stick/Shuffle answer — for correcting a registrant's
// own poll response after the fact (e.g. they meant the other option, or a raw-SQL fix like
// this needed a proper UI instead).
const stmtUpdateSignupTeamPref = db.prepare('UPDATE season_signups SET team_pref = ? WHERE id = ?');
export function updateSignupTeamPref(id, teamPref) { stmtUpdateSignupTeamPref.run(teamPref, id); }
...
export const withdrawSeasonSignup = db.transaction((signupId) => {
  ...
  stmtDeleteRosterSlot.run(signupId);
...
const stmtGetAssessmentsBySeason   = db.prepare('SELECT * FROM player_assessments WHERE season = ?');
...
export function getAssessmentsBySeason(season)               { return stmtGetAssessmentsBySeason.all(String(season)); }

// Each admin leaves their own tag/note/vote on a player's assessment — one row per
// (assessment, reviewer), replacing the single shared admin_tag/admin_note above (which stays
// in the schema only as the source for the one-time legacy backfill below; nothing writes to
// it anymore). reviewer_reg_id is never null: an individual player-admin uses their own
// reg id, and the shared super-admin login collapses to the fixed 'super' sentinel — SQLite's
// UNIQUE index treats NULL as distinct every time, which would silently defeat the "second
// person on the shared login overwrites the first" upsert behavior a real value gives us.
db.exec(`
  CREATE TABLE IF NOT EXISTS assessment_admin_reviews (
    id             TEXT    PRIMARY KEY,
    assessment_id  TEXT    NOT NULL,
    reviewer_reg_id TEXT   NOT NULL,
    ...
    UNIQUE(assessment_id, reviewer_reg_id)
  )
`);

const stmtUpsertAssessmentReview = db.prepare(`
  INSERT INTO assessment_admin_reviews (id, assessment_id, reviewer_reg_id, reviewer_name, tag, note, vote, created_at, updated_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  ON CONFLICT(assessment_id, reviewer_reg_id) DO UPDATE SET
    reviewer_name = excluded.reviewer_name, tag = excluded.tag, note = excluded.note,
    vote = excluded.vote, updated_at = excluded.updated_at
`);
const stmtGetAssessmentReviews = db.prepare('SELECT * FROM assessment_admin_reviews WHERE assessment_id = ? ORDER BY created_at ASC');
const stmtGetAssessmentReviewsBySeason = db.prepare(`
  SELECT r.* FROM assessment_admin_reviews r
  JOIN player_assessments a ON a.id = r.assessment_id
  WHERE a.season = ?
`);

export function upsertAssessmentReview(assessmentId, reviewerRegId, reviewerName, tag, note, vote) {
  const existing = db.prepare('SELECT id, created_at FROM assessment_admin_reviews WHERE assessment_id = ? AND reviewer_reg_id = ?').get(assessmentId, reviewerRegId);
  const now = Date.now();
  const id = existing?.id || `ar_${now}_${Math.random().toString(36).slice(2, 8)}`;
  stmtUpsertAssessmentReview.run(id, assessmentId, reviewerRegId, reviewerName, tag, note, vote, existing?.created_at ?? now, now);
}
export function getAssessmentReviews(assessmentId)           { return stmtGetAssessmentReviews.all(assessmentId); }
export function getAssessmentReviewsBySeason(season)          { return stmtGetAssessmentReviewsBySeason.all(String(season)); }

const stmtDeleteAssessmentReview = db.prepare('DELETE FROM assessment_admin_reviews WHERE id = ?');
export function deleteAssessmentReview(id) { stmtDeleteAssessmentReview.run(id); }

// One-time backfill of the old single shared tag/note into the new per-admin table, run at
// startup — deterministic 'legacy_<assessment.id>' id + INSERT OR IGNORE makes re-running on
// every boot a no-op once it's done. Only backfills assessments that actually had a tag/note
// entered; a never-reviewed assessment gets no legacy row (correctly shows 0 reviews).
{
  const stmtLegacyBackfill = db.prepare(`
    INSERT OR IGNORE INTO assessment_admin_reviews (id, assessment_id, reviewer_reg_id, reviewer_name, tag, note, vote, created_at, updated_at)
    SELECT 'legacy_' || id, id, 'legacy', 'Admin (legacy)', admin_tag, admin_note, '', created_at, created_at
    FROM player_assessments WHERE admin_tag != '' OR admin_note != ''
  `);
  stmtLegacyBackfill.run();
}
...
// The most recent season that actually has a drafted roster on file — a data-driven
// fallback for anything that wants "the season currently being previewed/played" without
// depending on signup_target_season staying in sync with it (that setting's lifecycle is
// admin-driven and can lag behind or get cleared independently of the roster data itself).
const stmtLatestRosterSeason = db.prepare(`SELECT season FROM season_roster ORDER BY CAST(season AS INTEGER) DESC LIMIT 1`);
export function getLatestRosterSeason()                   { return stmtLatestRosterSeason.get()?.season ?? ''; }
...
// Migrations for existing registrations table
{
  ...
  // JSON array of admin-nav section keys (see ADMIN_SECTIONS in views/admin/layout.js) an
  // elevated player-admin is restricted to — e.g. '["papawis"]' for an admin who should only
  // touch Papawis. Empty string (the default) means unrestricted, same as every admin before
  // this column existed. Never applies to the true super-admin login, which has no row here
  // to restrict in the first place.
  if (!_regCols.includes('admin_sections')) db.exec(`ALTER TABLE registrations ADD COLUMN admin_sections TEXT NOT NULL DEFAULT ''`);
  // Separate from admin_sections — having the 'marketplace' section only lets an elevated
  // admin create/manage listings; actually triggering a group-buy charge (real money moving)
  // needs this explicit second grant, same "extra capability flag on top of section access"
  // pattern as can_view_sensitive above.
  if (!_regCols.includes('can_charge_marketplace')) db.exec(`ALTER TABLE registrations ADD COLUMN can_charge_marketplace INTEGER NOT NULL DEFAULT 0`);
...
// Changes which player an already-approved registration points to, without touching status/
// approved_at/notes and without the approve email — for fixing a mislinked/mistakenly-created
// player after the fact (see updateRegistration, which would need every field re-passed and
// re-sends the approval email every time it runs).
const stmtRelinkRegPlayer = db.prepare('UPDATE registrations SET player_id = ? WHERE id = ?');
export function relinkRegistrationPlayer(id, playerId) {
  stmtRelinkRegPlayer.run(playerId, id);
}
...
const stmtSetMarketplaceChargeAccess = db.prepare('UPDATE registrations SET can_charge_marketplace=? WHERE id=?');
export function setRegistrationMarketplaceChargeAccess(id, allowed) { stmtSetMarketplaceChargeAccess.run(allowed ? 1 : 0, id); }

const stmtSetAdminSections = db.prepare('UPDATE registrations SET admin_sections=? WHERE id=?');
// sections: array of section keys, or empty/null to clear the restriction entirely.
export function setRegistrationAdminSections(id, sections) {
  const clean = Array.isArray(sections) ? sections.filter(Boolean) : [];
  stmtSetAdminSections.run(clean.length ? JSON.stringify(clean) : '', id);
}
// null = unrestricted (every section allowed) — the default, and always true for the
// shared super-admin login (no registrations row at all). Otherwise an array of keys.
export function getRegistrationAdminSections(reg) {
  if (!reg?.admin_sections) return null;
  try {
    const arr = JSON.parse(reg.admin_sections);
...
// season = '' rows (Papawis payments, misc "Other" entries, anything not tied to a
// specific season) are OR'd in rather than excluded by the exact match — otherwise
// selecting any specific season silently hides them entirely, which reads as "the
// transaction was never recorded" even though it's sitting right there in the ledger.
const stmtTxByPlayerSeason = db.prepare(`SELECT * FROM transaction_ledger WHERE player_id = ? AND (season = ? OR season = '') ORDER BY date DESC, created_at DESC`);
const stmtAllTx            = db.prepare('SELECT * FROM transaction_ledger ORDER BY date DESC, created_at DESC');
const stmtAllTxBySeason    = db.prepare(`SELECT * FROM transaction_ledger WHERE (season = ? OR season = '') ORDER BY date DESC, created_at DESC`);
...
const stmtAllSummary       = db.prepare(`
  ...
  FROM transaction_ledger
`);
// Season Fee is the one category the quota target is actually about — Papawis, Marketplace,
// etc. are real money collected but don't count toward "have they paid their season fee."
const stmtSeasonFeePaid   = db.prepare(`
  SELECT COALESCE(SUM(amount), 0) AS total
  FROM transaction_ledger
  WHERE season = ? AND category = 'Season Fee' AND type = 'payment' AND status = 'confirmed'
`);
...
// player_financials.current_balance rolls up every confirmed charge/payment regardless
// of category — season fees, penalties, Papawis deposits/charges, everything. That's
// right for the admin Ledger and profile balance display, but wrong for anything that
// gates Papawis participation specifically: a player who's fully square on Papawis but
// still paying off a season fee shouldn't be locked out of joining a pickup game. This
// sums only the Papawis-related slice (category starting with "Papawis" — covers both
// "Papawis" game charges and "Papawis Deposit"), same signed convention as current_balance
// (positive = owes, negative = credit on file).
const stmtGetPapawisBalance = db.prepare(`
  SELECT COALESCE(SUM(CASE WHEN type = 'charge' THEN amount WHEN type = 'payment' THEN -amount ELSE 0 END), 0) AS balance
  FROM transaction_ledger
  WHERE player_id = ? AND status = 'confirmed' AND category LIKE 'Papawis%'
`);
export function getPlayerPapawisBalance(playerId) {
  return stmtGetPapawisBalance.get(playerId)?.balance ?? 0;
}

// The actual join-blocker signal: a specific unpaid slot on a game that's already been
// closed out, not a netted ledger balance. A player who's settled every game they've
// actually played shouldn't stay blocked over ledger bookkeeping (deposit credits,
// payment/charge ordering, etc.) — this only fires on a real, concrete unpaid debt.
// Includes guest slots the player sponsored (they're who gets charged for those too).
// Waitlist/pending/cancelled signups never got charged at close-out, so they're excluded
// by the status='confirmed' filter — only a completed game's confirmed, unpaid slot counts.
const stmtHasUnpaidCompletedPapawis = db.prepare(`
  SELECT 1
  FROM papawis_signups s
  JOIN papawis_games g ON g.id = s.game_id
  WHERE s.player_id = ? AND s.status = 'confirmed' AND g.status = 'completed' AND s.paid_at IS NULL
  LIMIT 1
`);
export function hasUnpaidCompletedPapawis(playerId) {
  return !!stmtHasUnpaidCompletedPapawis.get(playerId);
}
...
export function getSeasonFeePaid(season)               { return stmtSeasonFeePaid.get(season)?.total ?? 0; }
...

{
  ...
  // Manual-only for now (v1): admin puts a player on the Papawis probation list (new
  // account with no clean papawis history, or a repeat late-payer) and takes them off —
  // no auto-detection yet. While set, joining papawis lands the player in a held 'pending'
  // signup state instead of the normal confirmed/waitlist flow — see joinPapawisGame().
  if (!_plCols.includes('papawis_probation')) db.exec(`ALTER TABLE players ADD COLUMN papawis_probation INTEGER NOT NULL DEFAULT 0`);
  if (!_plCols.includes('papawis_probation_note')) db.exec(`ALTER TABLE players ADD COLUMN papawis_probation_note TEXT NOT NULL DEFAULT ''`);
}
...
`);
{
  const _totalsCols = db.prepare("PRAGMA table_info(player_totals)").all().map(c => c.name);
  if (!_totalsCols.includes('fg4m')) db.exec(`ALTER TABLE player_totals ADD COLUMN fg4m INTEGER NOT NULL DEFAULT 0`);
  if (!_totalsCols.includes('fg4m_miss')) db.exec(`ALTER TABLE player_totals ADD COLUMN fg4m_miss INTEGER NOT NULL DEFAULT 0`);
}
...
db.exec(`
  CREATE TABLE IF NOT EXISTS games (
    ...
    team_a_to_team        INTEGER NOT NULL DEFAULT 0,
    team_b_to_team        INTEGER NOT NULL DEFAULT 0,
...
if (!_gamesCols.includes('team_a_to_team')) db.exec(`ALTER TABLE games ADD COLUMN team_a_to_team INTEGER NOT NULL DEFAULT 0`);
if (!_gamesCols.includes('team_b_to_team')) db.exec(`ALTER TABLE games ADD COLUMN team_b_to_team INTEGER NOT NULL DEFAULT 0`);
...
`);
{
  const _gpsCols = db.prepare("PRAGMA table_info(game_player_stats)").all().map(c => c.name);
  if (!_gpsCols.includes('fg4m')) db.exec(`ALTER TABLE game_player_stats ADD COLUMN fg4m INTEGER NOT NULL DEFAULT 0`);
  if (!_gpsCols.includes('fg4m_miss')) db.exec(`ALTER TABLE game_player_stats ADD COLUMN fg4m_miss INTEGER NOT NULL DEFAULT 0`);
}
...
// type='group_buy' rows only for now — type='player' listings + the comment tables land in
// Phase 2. min_buyers is a floor checked only at charge-trigger time, never a cap enforced on
// commit (unlike Papawis's max_slots, which is a ceiling) — see the trigger route in server.js.
db.exec(`
  CREATE TABLE IF NOT EXISTS marketplace_listings (
    id                   TEXT PRIMARY KEY,
    type                 TEXT NOT NULL,
    seller_id            TEXT NOT NULL DEFAULT '',
    created_by_reg_id    TEXT NOT NULL DEFAULT '',
    title                TEXT NOT NULL,
    description          TEXT NOT NULL DEFAULT '',
    ...
    status               TEXT NOT NULL DEFAULT 'active',
    min_buyers           INTEGER NOT NULL DEFAULT 0,
    variant_options      TEXT NOT NULL DEFAULT '[]',
    charge_triggered_at  INTEGER NOT NULL DEFAULT 0,
    charge_triggered_by  TEXT NOT NULL DEFAULT '',
    created_at           INTEGER NOT NULL,
    updated_at           INTEGER NOT NULL,
    sold_at              INTEGER NOT NULL DEFAULT 0
  )
`);
db.exec(`CREATE INDEX IF NOT EXISTS idx_marketplace_listings_type_status ON marketplace_listings(type, status)`);
{
  const _mlCols = db.prepare('PRAGMA table_info(marketplace_listings)').all().map(c => c.name);
  if (!_mlCols.includes('compare_at_price')) db.exec(`ALTER TABLE marketplace_listings ADD COLUMN compare_at_price REAL NOT NULL DEFAULT 0`);
  if (!_mlCols.includes('view_count')) db.exec(`ALTER TABLE marketplace_listings ADD COLUMN view_count INTEGER NOT NULL DEFAULT 0`);
  if (!_mlCols.includes('goal_group_id')) db.exec(`ALTER TABLE marketplace_listings ADD COLUMN goal_group_id TEXT NOT NULL DEFAULT ''`);
}

// Lets separate, independently-priced/photographed listings pool one shared "how many
// buyers so far" progress bar — e.g. two different jersey designs sold as their own
// listings that should still read as one combined commit goal. A listing with a non-empty
// goal_group_id has its committed count/avatars/threshold computed from every listing
// sharing that group id instead of itself alone (see getGroupCommittedPlayers below);
// everything else about it (title, photos, variants, price) stays fully independent.
db.exec(`
  CREATE TABLE IF NOT EXISTS marketplace_listing_groups (
    id          TEXT PRIMARY KEY,
...
export function createMarketplaceListingGroup({ label = '', minBuyers = 0 }) {
  const id = crypto.randomUUID();
  db.prepare('INSERT INTO marketplace_listing_groups (id, label, min_buyers, created_at) VALUES (?, ?, ?, ?)')
    .run(id, label, Number(minBuyers) || 0, Date.now());
  return id;
}

export function getMarketplaceListingGroups() {
  return db.prepare('SELECT * FROM marketplace_listing_groups ORDER BY created_at DESC').all();
}

export function getMarketplaceListingGroupById(id) {
  return db.prepare('SELECT * FROM marketplace_listing_groups WHERE id = ?').get(id) || null;
}

export function updateMarketplaceListingGroup(id, { label, minBuyers }) {
  db.prepare('UPDATE marketplace_listing_groups SET label = ?, min_buyers = ? WHERE id = ?')
    .run(label, Number(minBuyers) || 0, id);
}

// Every committed row across every listing that shares this goal_group_id — the pooled
// equivalent of getMarketplaceCommitments for a single listing.
const stmtGetGroupCommittedPlayers = db.prepare(`
  SELECT c.*, p.name AS player_name, p.picture_url, t.name AS team_name
  FROM marketplace_commitments c
  JOIN players p ON p.id = c.player_id
  LEFT JOIN teams t ON t.id = p.team_id
  JOIN marketplace_listings l ON l.id = c.listing_id
  WHERE l.goal_group_id = ? AND c.status = 'committed'
  ORDER BY c.committed_at ASC
`);
export function getGroupCommittedPlayers(groupId) {
  return stmtGetGroupCommittedPlayers.all(groupId);
}

// No UNIQUE(listing_id, player_id) — rows are never deleted (a player can cancel then
// re-commit, and history should survive), same reasoning as papawis_signups never being
// hard-deleted. "Already has an active commitment" is enforced in application code via
// stmtActiveMarketplaceCommitment below, mirroring the active-papawis-signup check pattern.
db.exec(`
  CREATE TABLE IF NOT EXISTS marketplace_commitments (
    id             TEXT PRIMARY KEY,
...
`);
db.exec(`CREATE INDEX IF NOT EXISTS idx_marketplace_commitments_listing ON marketplace_commitments(listing_id)`);
{
  const _mcCols = db.prepare('PRAGMA table_info(marketplace_commitments)').all().map(c => c.name);
  if (!_mcCols.includes('custom_name'))   db.exec(`ALTER TABLE marketplace_commitments ADD COLUMN custom_name TEXT NOT NULL DEFAULT ''`);
  if (!_mcCols.includes('custom_number')) db.exec(`ALTER TABLE marketplace_commitments ADD COLUMN custom_number TEXT NOT NULL DEFAULT ''`);
  if (!_mcCols.includes('notes'))         db.exec(`ALTER TABLE marketplace_commitments ADD COLUMN notes TEXT NOT NULL DEFAULT ''`);
}

const stmtInsertMarketplaceListing = db.prepare(`
  INSERT INTO marketplace_listings
    (id, type, seller_id, created_by_reg_id, title, description, price, compare_at_price, photos, status,
     min_buyers, variant_options, goal_group_id, created_at, updated_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
export function createMarketplaceListing({
  type, sellerId = '', createdByRegId = '', title, description = '', price = 0, compareAtPrice = 0,
  photos = [], status = 'active', minBuyers = 0, variantOptions = [], goalGroupId = '',
}) {
  const id = crypto.randomUUID();
  const now = Date.now();
  stmtInsertMarketplaceListing.run(
    id, type, sellerId, createdByRegId, title, description, Number(price) || 0, Number(compareAtPrice) || 0,
    JSON.stringify(photos), status, Number(minBuyers) || 0, JSON.stringify(variantOptions), goalGroupId || '',
    now, now
  );
  return id;
}

const stmtGetMarketplaceListingById = db.prepare('SELECT * FROM marketplace_listings WHERE id = ?');
export function getMarketplaceListingById(id) { return stmtGetMarketplaceListingById.get(id) || null; }

export function getMarketplaceListings({ type = '', status = '' } = {}) {
...
const stmtUpdateMarketplaceListing = db.prepare(`
  UPDATE marketplace_listings
  SET title = ?, description = ?, price = ?, compare_at_price = ?, min_buyers = ?, variant_options = ?, goal_group_id = ?, updated_at = ?
  WHERE id = ?
`);
export function updateMarketplaceListing(id, { title, description = '', price = 0, compareAtPrice = 0, minBuyers = 0, variantOptions = [], goalGroupId = '' }) {
  stmtUpdateMarketplaceListing.run(title, description, Number(price) || 0, Number(compareAtPrice) || 0, Number(minBuyers) || 0, JSON.stringify(variantOptions), goalGroupId || '', Date.now(), id);
}

// Raw hit counter, incremented once per detail-page load (no dedup by viewer/session) — kept
// deliberately simple, same "just count the requests" spirit as everything else in this file
// that isn't trying to be full analytics. Doesn't bump updated_at, since a view isn't an edit.
const stmtIncrementMarketplaceListingViews = db.prepare('UPDATE marketplace_listings SET view_count = view_count + 1 WHERE id = ?');
export function incrementMarketplaceListingViews(id) { stmtIncrementMarketplaceListingViews.run(id); }

const stmtSetMarketplaceListingStatus = db.prepare('UPDATE marketplace_listings SET status = ?, updated_at = ? WHERE id = ?');
export function setMarketplaceListingStatus(id, status) { stmtSetMarketplaceListingStatus.run(status, Date.now(), id); }

const stmtSetMarketplaceListingPhotos = db.prepare('UPDATE marketplace_listings SET photos = ?, updated_at = ? WHERE id = ?');
export function setMarketplaceListingPhotos(id, photos) { stmtSetMarketplaceListingPhotos.run(JSON.stringify(photos), Date.now(), id); }

// Variant-options-only update — used when a photo delete/reorder needs to re-point
// photo-backed variant options at their new indices without touching title/price/etc.
const stmtSetMarketplaceListingVariantOptions = db.prepare('UPDATE marketplace_listings SET variant_options = ?, updated_at = ? WHERE id = ?');
export function setMarketplaceListingVariantOptions(id, variantOptions) { stmtSetMarketplaceListingVariantOptions.run(JSON.stringify(variantOptions), Date.now(), id); }

const stmtChargeMarketplaceListing = db.prepare(`
  UPDATE marketplace_listings SET status = 'charged', charge_triggered_at = ?, charge_triggered_by = ?, updated_at = ? WHERE id = ?
`);
export function markMarketplaceListingCharged(id, triggeredBy) {
  const now = Date.now();
  stmtChargeMarketplaceListing.run(now, triggeredBy, now, id);
}

const stmtDeleteMarketplaceListing = db.prepare('DELETE FROM marketplace_listings WHERE id = ?');
export function deleteMarketplaceListing(id) { stmtDeleteMarketplaceListing.run(id); }

const stmtInsertMarketplaceCommitment = db.prepare(`
  INSERT INTO marketplace_commitments (id, listing_id, player_id, variant, custom_name, custom_number, notes, status, committed_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, 'committed', ?)
`);
export function commitToMarketplaceListing(listingId, playerId, variant = '', { customName = '', customNumber = '', notes = '' } = {}) {
  const id = crypto.randomUUID();
  stmtInsertMarketplaceCommitment.run(id, listingId, playerId, variant, customName, customNumber, notes, Date.now());
  return id;
}

const stmtActiveMarketplaceCommitment = db.prepare(`
  SELECT * FROM marketplace_commitments WHERE listing_id = ? AND player_id = ? AND status = 'committed'
`);
export function getActiveMarketplaceCommitment(listingId, playerId) {
  return stmtActiveMarketplaceCommitment.get(listingId, playerId) || null;
}

// A player can hold several active commitments on the same listing — e.g. one photo-backed
// grid explodes a jersey design into its own card, and a buyer ordering both a Bucks and a
// Chicago jersey ends up with two separate line items here, each with its own variant
// selections/custom name/number. getActiveMarketplaceCommitment above still returns just one
// (used only for "does this player have anything at all" existence checks); this is the
// full list, oldest first, used anywhere the actual order needs to be shown or totaled.
const stmtActiveMarketplaceCommitmentsForPlayer = db.prepare(`
  SELECT * FROM marketplace_commitments WHERE listing_id = ? AND player_id = ? AND status = 'committed' ORDER BY committed_at ASC
`);
export function getActiveMarketplaceCommitmentsForPlayer(listingId, playerId) {
  return stmtActiveMarketplaceCommitmentsForPlayer.all(listingId, playerId);
}

const stmtGetMarketplaceCommitmentById = db.prepare('SELECT * FROM marketplace_commitments WHERE id = ?');
export function getMarketplaceCommitmentById(id) { return stmtGetMarketplaceCommitmentById.get(id) || null; }

const stmtCancelMarketplaceCommitment = db.prepare(`
  UPDATE marketplace_commitments SET status = 'cancelled', cancelled_at = ? WHERE id = ?
`);
export function cancelMarketplaceCommitment(id) { stmtCancelMarketplaceCommitment.run(Date.now(), id); }

const stmtGetMarketplaceCommitments = db.prepare(`
  SELECT c.*, p.name AS player_name, p.picture_url, t.name AS team_name
  FROM marketplace_commitments c
  JOIN players p ON p.id = c.player_id
  LEFT JOIN teams t ON t.id = p.team_id
  WHERE c.listing_id = ? AND c.status = ?
  ORDER BY c.committed_at ASC
`);
export function getMarketplaceCommitments(listingId, status = 'committed') {
  return stmtGetMarketplaceCommitments.all(listingId, status);
}

const stmtCountActiveMarketplaceCommitments = db.prepare(`
  SELECT COUNT(*) AS n FROM marketplace_commitments WHERE listing_id = ? AND status = 'committed'
`);
export function countActiveMarketplaceCommitments(listingId) {
  return stmtCountActiveMarketplaceCommitments.get(listingId)?.n ?? 0;
}

const stmtMarkMarketplaceCommitmentCharged = db.prepare(`
  UPDATE marketplace_commitments SET status = 'charged', charged_at = ?, transaction_id = ? WHERE id = ?
`);
export function markMarketplaceCommitmentCharged(id, txId) {
  stmtMarkMarketplaceCommitmentCharged.run(Date.now(), txId, id);
}

// ── Marketplace comments + reactions ────────────────────────────────────────────
// Exact parallel of game_comments/comment_reactions/game_reactions above, listing_id in
// place of game_id — its own parallel tables rather than a polymorphic target, same
// reasoning as game_reactions' own comment there. No @mentions here (unlike game
// comments) — marketplace listings don't have a "who played" pool to mention.
db.exec(`
...
`);
db.exec(`CREATE INDEX IF NOT EXISTS idx_marketplace_listing_comments_listing ON marketplace_listing_comments(listing_id)`);
...
const stmtInsertMarketplaceComment = db.prepare(`
  INSERT INTO marketplace_listing_comments (id, listing_id, player_id, body, created_at) VALUES (?, ?, ?, ?, ?)
`);
const stmtDeleteMarketplaceComment = db.prepare('DELETE FROM marketplace_listing_comments WHERE id = ?');
const stmtDeleteMarketplaceCommentReactions = db.prepare('DELETE FROM marketplace_comment_reactions WHERE comment_id = ?');
const stmtGetMarketplaceListingComments = db.prepare(`
  SELECT c.id, c.listing_id, c.player_id, c.body, c.created_at,
    p.name AS player_name, p.team_id, t.name AS team_name,
    (SELECT COUNT(*) FROM marketplace_comment_reactions r WHERE r.comment_id = c.id) AS reaction_count
  ...
  ORDER BY c.created_at ASC
`);
const stmtGetMarketplaceCommentById = db.prepare('SELECT * FROM marketplace_listing_comments WHERE id = ?');
// Same joined shape as stmtGetMarketplaceListingComments, single row — used to build the
// WebSocket broadcast payload right after a comment is inserted.
const stmtGetMarketplaceCommentWithMeta = db.prepare(`
  SELECT c.id, c.listing_id, c.player_id, c.body, c.created_at,
    p.name AS player_name, p.team_id, t.name AS team_name,
    (SELECT COUNT(*) FROM marketplace_comment_reactions r WHERE r.comment_id = c.id) AS reaction_count
  FROM marketplace_listing_comments c
  ...
  WHERE c.id = ?
`);
const stmtGetMarketplaceCommentReaction = db.prepare('SELECT 1 FROM marketplace_comment_reactions WHERE comment_id = ? AND player_id = ?');
const stmtInsertMarketplaceCommentReaction = db.prepare('INSERT OR IGNORE INTO marketplace_comment_reactions (comment_id, player_id, created_at) VALUES (?, ?, ?)');
const stmtDeleteMarketplaceCommentReaction = db.prepare('DELETE FROM marketplace_comment_reactions WHERE comment_id = ? AND player_id = ?');
const stmtCountMarketplaceCommentReactions = db.prepare('SELECT COUNT(*) AS n FROM marketplace_comment_reactions WHERE comment_id = ?');

export function getMarketplaceListingComments(listingId) { return stmtGetMarketplaceListingComments.all(listingId); }
export function getMarketplaceCommentById(id)             { return stmtGetMarketplaceCommentById.get(id); }
export function getMarketplaceCommentWithMeta(id)         { return stmtGetMarketplaceCommentWithMeta.get(id); }

export function addMarketplaceComment({ listingId, playerId, body }) {
  const id = crypto.randomUUID();
  stmtInsertMarketplaceComment.run(id, listingId, playerId, body, Date.now());
  return id;
}

export function deleteMarketplaceComment(id) {
  stmtDeleteMarketplaceCommentReactions.run(id);
  stmtDeleteMarketplaceComment.run(id);
}

export function toggleMarketplaceCommentReaction(commentId, playerId) {
  const already = !!stmtGetMarketplaceCommentReaction.get(commentId, playerId);
  if (already) stmtDeleteMarketplaceCommentReaction.run(commentId, playerId);
  else stmtInsertMarketplaceCommentReaction.run(commentId, playerId, Date.now());
  return { reacted: !already, count: stmtCountMarketplaceCommentReactions.get(commentId).n };
}

export function getReactedMarketplaceCommentIdsForPlayer(commentIds, playerId) {
  if (!playerId || !commentIds.length) return new Set();
  const placeholders = commentIds.map(() => '?').join(',');
  const rows = db.prepare(`SELECT comment_id FROM marketplace_comment_reactions WHERE player_id = ? AND comment_id IN (${placeholders})`).all(playerId, ...commentIds);
  return new Set(rows.map(r => r.comment_id));
}

export function getMarketplaceCommentCounts(listingIds) {
  if (!listingIds.length) return {};
  const placeholders = listingIds.map(() => '?').join(',');
  const rows = db.prepare(`SELECT listing_id, COUNT(*) AS n FROM marketplace_listing_comments WHERE listing_id IN (${placeholders}) GROUP BY listing_id`).all(...listingIds);
  return Object.fromEntries(rows.map(r => [r.listing_id, r.n]));
}

// Page-level "like this listing" reaction — same shape/reasoning as game_reactions above.
db.exec(`
  CREATE TABLE IF NOT EXISTS marketplace_listing_reactions (
    listing_id TEXT NOT NULL,
    player_id  TEXT NOT NULL,
    created_at INTEGER NOT NULL,
...
`);
const stmtGetMarketplaceListingReaction    = db.prepare('SELECT 1 FROM marketplace_listing_reactions WHERE listing_id = ? AND player_id = ?');
const stmtInsertMarketplaceListingReaction = db.prepare('INSERT OR IGNORE INTO marketplace_listing_reactions (listing_id, player_id, created_at) VALUES (?, ?, ?)');
const stmtDeleteMarketplaceListingReaction = db.prepare('DELETE FROM marketplace_listing_reactions WHERE listing_id = ? AND player_id = ?');
const stmtCountMarketplaceListingReactions = db.prepare('SELECT COUNT(*) AS n FROM marketplace_listing_reactions WHERE listing_id = ?');

export function toggleMarketplaceListingReaction(listingId, playerId) {
  const already = !!stmtGetMarketplaceListingReaction.get(listingId, playerId);
  if (already) stmtDeleteMarketplaceListingReaction.run(listingId, playerId);
  else stmtInsertMarketplaceListingReaction.run(listingId, playerId, Date.now());
  return { reacted: !already, count: stmtCountMarketplaceListingReactions.get(listingId).n };
}

export function getMarketplaceListingReactionState(listingId, playerId) {
  return {
    count: stmtCountMarketplaceListingReactions.get(listingId).n,
    reacted: playerId ? !!stmtGetMarketplaceListingReaction.get(listingId, playerId) : false,
  };
}

export function getMarketplaceListingReactionCounts(listingIds) {
  if (!listingIds.length) return {};
  const placeholders = listingIds.map(() => '?').join(',');
  const rows = db.prepare(`SELECT listing_id, COUNT(*) AS n FROM marketplace_listing_reactions WHERE listing_id IN (${placeholders}) GROUP BY listing_id`).all(...listingIds);
  return Object.fromEntries(rows.map(r => [r.listing_id, r.n]));
}
...
// Same as getHeadToHeadRecord, but scoped to one season's regular-season games only — used
// as a standings/seeding tiebreaker, where a result from a different season (or a playoff
// game) shouldn't count.
const stmtGetHeadToHeadSeason = db.prepare(`
  SELECT date, team_a_id, team_a_score, team_b_id, team_b_score
  FROM games
  WHERE status = 'complete' AND under_review = 0 AND season = ? AND game_type = 'regular'
    AND ((team_a_id = ? AND team_b_id = ?) OR (team_a_id = ? AND team_b_id = ?))
  ORDER BY date ASC
`);
export function getHeadToHeadRecordForSeason(teamAId, teamBId, season) {
  const games = stmtGetHeadToHeadSeason.all(String(season), teamAId, teamBId, teamBId, teamAId);
  let teamAWins = 0, teamBWins = 0;
  for (const g of games) {
    const aScore = g.team_a_id === teamAId ? g.team_a_score : g.team_b_score;
    const bScore = g.team_a_id === teamAId ? g.team_b_score : g.team_a_score;
  ...
  return { teamAId, teamBId, teamAWins, teamBWins, games };
}
...
const stmtGetTeamSeasonStats = db.prepare(`
  SELECT gps.team_id, t.name AS team_name,
         ...
         COALESCE(SUM(gps.fg4m), 0)                                              AS fg4m,
         COALESCE(SUM(gps.fg2m + gps.fg3m + gps.fg4m), 0)                       AS fgm,
         COALESCE(SUM(gps.fg2m + gps.fg3m + gps.fg4m + gps.fg2m_miss + gps.fg3m_miss + gps.fg4m_miss), 0) AS fga,
         COALESCE(SUM(gps.fg3m + gps.fg3m_miss), 0)                             AS fg3a,
         COALESCE(SUM(gps.fg4m + gps.fg4m_miss), 0)                             AS fg4a,
...
// Turnovers attributed to the team as a whole (shot clock violations, etc.),
// not to any individual player — kept out of game_player_stats/player_totals
// entirely so leaders/ratings/PER are unaffected. Merged into team season
// totals only (see getTeamSeasonStats).
const stmtGetTeamOnlyTurnovers = db.prepare(`
  SELECT team_a_id AS team_id, COALESCE(SUM(team_a_to_team), 0) AS turnover
  FROM games WHERE season = ? AND game_type = 'regular' AND status = 'complete'
  GROUP BY team_a_id
  UNION ALL
  SELECT team_b_id AS team_id, COALESCE(SUM(team_b_to_team), 0) AS turnover
  FROM games WHERE season = ? AND game_type = 'regular' AND status = 'complete'
  GROUP BY team_b_id
`);
...
// Points for/against per team, league-wide — same team_a/team_b union shape as
// stmtGetTeamRecords, just summing scores instead of counting wins. Used for the team page's
// "Defense" rank card (points allowed per game) alongside every other team's, not just the
// one team server.js's team-detail route otherwise computes this for.
const stmtGetTeamPointsForAgainst = db.prepare(`
  SELECT team_id, SUM(team_score) AS points_for, SUM(opp_score) AS points_against
  FROM (
    SELECT team_a_id AS team_id, team_a_score AS team_score, team_b_score AS opp_score FROM games
    WHERE season = ?
      AND game_type = 'regular' AND status = 'complete' AND under_review = 0
...
const stmtGetLeaders = db.prepare(`
  SELECT p.id, p.name, p.team_id, p.picture_url,
         ...
         COALESCE(SUM(gps.fg4m_miss), 0)   AS fg4m_miss,
         COALESCE(SUM(gps.ftm), 0)         AS ftm,
         COALESCE(SUM(gps.ft_miss), 0)     AS ft_miss
  FROM players p
...
const stmtGetLeadersAllTime = db.prepare(`
  SELECT p.id, p.name, p.team_id, p.picture_url,
         tm.name AS team_name,
         COUNT(DISTINCT gps.game_id)       AS games_played,
         COALESCE(SUM(gps.pts), 0)         AS pts,
...
const stmtGetLeaderSeasons = db.prepare(`
  SELECT DISTINCT season FROM games
  WHERE game_type = 'regular' AND status = 'complete'
...
const stmtGetGameRecords = db.prepare(`
  SELECT
    ...
    gps.fg2m, gps.fg3m, gps.fg4m, gps.fg2m_miss, gps.fg3m_miss, gps.fg4m_miss,
...
const stmtGetGameById = db.prepare(`
  SELECT id, date, team_a_id, team_b_id, team_a_name, team_b_name,
         team_a_score, team_b_score, team_a_to_team, team_b_to_team,
         game_writeup, potg_writeup,
...
const stmtGetGameDetailStats = db.prepare(`
  SELECT gps.player_id, gps.team_id,
         ...
         gps.fg2m, gps.fg3m, gps.fg4m, gps.fg2m_miss, gps.fg3m_miss, gps.fg4m_miss, gps.ftm, gps.ft_miss, gps.minutes,
...
const stmtGetGameStats = db.prepare(`
  SELECT player_id, team_id, pts, ast, reb, stl, blk, turnover,
         fg2m, fg3m, fg4m, fg2m_miss, fg3m_miss, fg4m_miss, ftm, ft_miss, minutes
  ...
  SELECT g.id, g.date, g.season, g.game_type,
         ...
         COALESCE(
           ...
           (SELECT gps2.team_id
            ...
            WHERE gps2.player_id = p.id AND g2.season = g.season AND g2.status = 'complete'
              AND ABS(julianday(g2.date) - julianday(g.date)) = (
                SELECT MIN(ABS(julianday(g3.date) - julianday(g.date)))
                FROM game_player_stats gps3
                JOIN games g3 ON g3.id = gps3.game_id
                WHERE gps3.player_id = p.id AND g3.season = g.season AND g3.status = 'complete'
              )
            LIMIT 1),
           p.team_id
         ) AS player_team_id,
...
const stmtGetPlayerPotgCandidates = db.prepare(`
  SELECT g.id, g.date, g.team_a_id, g.team_a_name, g.team_a_score,
         ...
         gps.pts, gps.reb, gps.ast, gps.stl, gps.blk,
         gps.fg2m, gps.fg3m, gps.fg4m, gps.fg2m_miss, gps.fg3m_miss, gps.fg4m_miss, gps.ftm, gps.ft_miss
  FROM games g
...
const stmtGetPlayerCareerHighs = db.prepare(`
  SELECT MAX(pts) AS pts, MAX(reb) AS reb, MAX(ast) AS ast,
         MAX(stl) AS stl, MAX(blk) AS blk, MAX(fg3m) AS fg3m, MAX(fg4m) AS fg4m
...
// How many regular-season games a team played in a season — the denominator for the
// Iron Man badge (games_played === this, not some fixed schedule-length constant, since
// season length varies).
const stmtGetTeamGamesCount = db.prepare(`
  SELECT COUNT(*) AS c FROM games
  WHERE season = ? AND game_type = 'regular' AND status = 'complete'
...
const stmtGetGameDnpPlayers = db.prepare(`
  SELECT p.id, p.name, dt.resolved_team_id AS team_id, t.name AS team_name
  FROM (
    SELECT d.player_id,
           COALESCE(
             (SELECT gps2.team_id
              FROM game_player_stats gps2
              JOIN games g2 ON g2.id = gps2.game_id
              WHERE gps2.player_id = d.player_id AND g2.season = g.season AND g2.status = 'complete'
                AND ABS(julianday(g2.date) - julianday(g.date)) = (
                  SELECT MIN(ABS(julianday(g3.date) - julianday(g.date)))
                  FROM game_player_stats gps3
                  JOIN games g3 ON g3.id = gps3.game_id
                  WHERE gps3.player_id = d.player_id AND g3.season = g.season AND g3.status = 'complete'
                )
              LIMIT 1),
             (SELECT team_id FROM players WHERE id = d.player_id)
           ) AS resolved_team_id
    FROM game_dnp d
  ...
  JOIN teams t ON t.id = dt.resolved_team_id
...
// Fields an admin can hand-correct on a single box score line. `pts` is deliberately excluded —
// it's always re-derived from the made-shot fields below so it can't drift out of sync with them.
export const EDITABLE_STAT_FIELDS = new Set([
  'ast', 'reb', 'stl', 'blk', 'turnover', 'pf',
  'fg2m', 'fg3m', 'fg4m', 'fg2m_miss', 'fg3m_miss', 'fg4m_miss', 'ftm', 'ft_miss',
]);

const stmtGetGamePlayerStat = db.prepare('SELECT * FROM game_player_stats WHERE game_id = ? AND player_id = ?');
export function getGamePlayerStat(gameId, playerId) { return stmtGetGamePlayerStat.get(gameId, playerId); }

const stmtRecalcPts = db.prepare(`
  UPDATE game_player_stats SET pts = fg2m*2 + fg3m*3 + fg4m*4 + ftm
  WHERE game_id = ? AND player_id = ?
`);

// One-field correction to an already-imported box score line, for fixing a single mistyped
// stat without re-importing (and overwriting) the whole game. pts is recomputed from the made-
// shot fields on every call so a fg2m/fg3m/fg4m/ftm edit can never leave pts stale.
export function updateGamePlayerStat(gameId, playerId, field, value) {
  if (!EDITABLE_STAT_FIELDS.has(field)) throw new Error(`Field "${field}" is not editable`);
  const val = Number(value) | 0;
  db.prepare(`UPDATE game_player_stats SET ${field} = ? WHERE game_id = ? AND player_id = ?`).run(val, gameId, playerId);
  stmtRecalcPts.run(gameId, playerId);
  const game = db.prepare('SELECT season FROM games WHERE id = ?').get(gameId);
  recomputePlayerTotals(game?.season ?? null);
  return stmtGetGamePlayerStat.get(gameId, playerId);
}

const stmtGetLogsForGame = db.prepare(`
  SELECT * FROM admin_logs WHERE path LIKE ? ORDER BY created_at DESC LIMIT ?
`);
export function getStatCorrectionsForGame(gameId, limit = 50) {
  return stmtGetLogsForGame.all(`/admin/games/${gameId}/stats/%`, limit);
}
export function setGameOvertime(gameId, overtime) {
  stmtSetOvertime.run(Number(overtime) || 0, gameId);
}

export function importGameResults(gameId, { teamAScore, teamBScore, periodSnapshots, gameLog, dnpPlayerIds, playerStats, season, teamATeamTurnovers, teamBTeamTurnovers }) {
  ...
  db.transaction(() => {
    ...
    stmtImportScores.run(
      ...
      Number(teamATeamTurnovers) || 0, Number(teamBTeamTurnovers) || 0,
      gameId
    );
    for (const [playerId, s] of Object.entries(playerStats || {})) {
      ...
      stmtInsertGameStat.run({
        ...
        fg2m: s.fg2m|0, fg3m: s.fg3m|0, fg4m: s.fg4m|0,
        fg2m_miss: s.fg2m_miss|0, fg3m_miss: s.fg3m_miss|0, fg4m_miss: s.fg4m_miss|0,
...
export function getTeamSeasonStats(season) {
  const rows = stmtGetTeamSeasonStats.all(String(season));
  const teamOnly = {};
  for (const r of stmtGetTeamOnlyTurnovers.all(String(season), String(season))) {
    teamOnly[r.team_id] = (teamOnly[r.team_id] || 0) + r.turnover;
  }
  for (const r of rows) r.turnover += teamOnly[r.team_id] || 0;
  return rows;
}
export function getTeamRecords(season)              { return stmtGetTeamRecords.all(String(season), String(season)); }
export function getTeamPointsForAgainst(season)     { return stmtGetTeamPointsForAgainst.all(String(season), String(season)); }
export function getTeamRecordsAsOf(season, date)   { return stmtGetTeamRecordsAsOf.all(season, date, season, date); }
// A player who has since moved teams still shows the team they actually played that season
// for, not their current live team — same "team a player earned it with" rule getPlayerTeamIdForSeason
// applies to awards (see below). Defined further down in this file but hoisted, so usable here.
function withSeasonTeam(p, season) {
  const teamId = getPlayerTeamIdForSeason(p.id, season);
  if (!teamId || teamId === p.team_id) return p;
  const team = stmtGetTeamById.get(teamId);
  return team ? { ...p, team_id: teamId, team_name: team.name } : p;
}
export function getLeaders(season)                  { return stmtGetLeaders.all(String(season)).map(p => withSeasonTeam(p, season)); }
export function getPlayoffLeaders(season)           { return stmtGetPlayoffLeaders.all(String(season)).map(p => withSeasonTeam(p, season)); }
export function getLeadersAllTime()                 { return stmtGetLeadersAllTime.all(); }
export function getLeaderSeasons()                  { return stmtGetLeaderSeasons.all().map(r => r.season); }
...
export function getTeamGamesCount(teamId, season)   { return stmtGetTeamGamesCount.get(String(season), teamId, teamId)?.c ?? 0; }
...
const stmtGetAllPlayerCareerTotals = db.prepare(`
  SELECT gps.player_id,
         ...
         SUM(gps.fg2m) AS fg2m, SUM(gps.fg3m) AS fg3m, SUM(gps.fg4m) AS fg4m,
         SUM(gps.fg2m_miss) AS fg2m_miss, SUM(gps.fg3m_miss) AS fg3m_miss, SUM(gps.fg4m_miss) AS fg4m_miss,
...
const stmtGetSeasonPlayerStats = db.prepare(`
  SELECT p.id, p.name, p.positions, t.name AS team_name, t.id AS team_id, t.color AS team_color, p.picture_url,
         ...
         SUM(gps.fg4m)      AS fg4m, SUM(gps.fg4m_miss) AS fg4m_miss,
         SUM(gps.ftm)       AS ftm,  SUM(gps.ft_miss)   AS ft_miss,
...
const stmtGetPlayerSeasonTeamId = db.prepare(`
  SELECT gps.team_id
  FROM game_player_stats gps
  JOIN games g ON g.id = gps.game_id
  WHERE gps.player_id = ? AND g.season = ? AND g.status = 'complete'
  ORDER BY g.date DESC, g.id DESC
  LIMIT 1
`);
// Team the player actually played for in the given season (their most recent played
// game that season), not their current live team — awards must reflect the team a
// player earned the award with, even after a later trade moves them elsewhere.
export function getPlayerTeamIdForSeason(playerId, season) {
  const row = stmtGetPlayerSeasonTeamId.get(playerId, season);
  if (row) return row.team_id;
  const player = stmtGetPlayerById.get(playerId);
  return player ? player.team_id : '';
}

export function upsertAward({ id, season, award_type, player_id, team_id = '', notes = '' }) {
  const resolvedTeamId = team_id || getPlayerTeamIdForSeason(player_id, season);
  stmtUpsertAward.run(id, season, award_type, player_id, resolvedTeamId, notes, Date.now());
}
...
// The team a player was actually on as of their most recent real game before the given
// season — used to detect "new"/"traded" for a season that's already started, where
// players.team_id has already been synced to match the *new* roster (see the "Start
// Season" flow) and season_roster from an earlier season may not even exist (the draft
// tool postdates some past seasons). Game history is untouched by either of those and
// goes back further, so it's the only durable source for "what team were they on before."
const stmtPlayerLastTeamBeforeSeason = db.prepare(`
  SELECT gps.team_id
  FROM game_player_stats gps
  JOIN games g ON g.id = gps.game_id
  WHERE gps.player_id = ? AND g.season < ?
  ORDER BY g.date DESC, g.id DESC
  LIMIT 1
`);
export function getPlayerLastTeamIdBeforeSeason(playerId, season) {
  return stmtPlayerLastTeamBeforeSeason.get(playerId, Number(season))?.team_id || '';
}

const stmtSetPlayerProbation = db.prepare(`UPDATE players SET papawis_probation = ?, papawis_probation_note = ? WHERE id = ?`);
export function setPlayerPapawisProbation(id, on, note = '') {
  stmtSetPlayerProbation.run(on ? 1 : 0, on ? String(note || '').trim() : '', id);
}
...
const stmtGetAllPlayerHeights = db.prepare(`SELECT player_id, height FROM player_details WHERE height IS NOT NULL AND height != ''`);
export function getAllPlayerHeights() { return stmtGetAllPlayerHeights.all(); }
...
export function mergeRegistrationIntoPlayer(playerId, reg) {
  db.transaction(() => {
    ...
    // upsertPlayerDetails writes every field it's given, blanking whatever's omitted (the
    // admin bio editor relies on that to let an admin clear a field) — so a registration,
    // which only ever supplies these three, must carry the player's other existing details
    // through untouched rather than let them fall through to upsertPlayerDetails as blank.
    if (reg.height || reg.weight || reg.dominant_hand) {
      const existing = stmtGetPlayerWithTeam.get(playerId) || {};
      upsertPlayerDetails(playerId, {
        nickname:         existing.nickname,
        hometown:         existing.hometown,
        school:           existing.school,
        wingspan:         existing.wingspan,
        years_playing:    existing.years_playing,
        social_instagram: existing.social_instagram,
        social_twitter:   existing.social_twitter,
        height:        reg.height        || existing.height,
        weight:        reg.weight        || existing.weight,
        dominant_hand: reg.dominant_hand || existing.dominant_hand,
      });
    }
  })();
}
...
export function recomputePlayerTotals(season) {
  ...
  const rows = db.prepare(`
    SELECT player_id,
      ...
      SUM(fg2m) AS fg2m, SUM(fg3m) AS fg3m, SUM(fg4m) AS fg4m,
      SUM(fg2m_miss) AS fg2m_miss, SUM(fg3m_miss) AS fg3m_miss, SUM(fg4m_miss) AS fg4m_miss,
  ...
  const upsert = db.prepare(`
    ...
    ON CONFLICT(player_id, season) DO UPDATE SET
      ...
      fg3m=excluded.fg3m, fg4m=excluded.fg4m, fg2m_miss=excluded.fg2m_miss,
      fg3m_miss=excluded.fg3m_miss, fg4m_miss=excluded.fg4m_miss,
      ftm=excluded.ftm, ft_miss=excluded.ft_miss
  `);
  const run = db.transaction(() => {
    for (const r of rows) {
      upsert.run(r.player_id, s, r.games_played, r.pts, r.ast, r.reb, r.stl, r.blk,
        r.turnover, r.pf, r.fg2m, r.fg3m, r.fg4m, r.fg2m_miss, r.fg3m_miss, r.fg4m_miss, r.ftm, r.ft_miss);
    }
  });
  run();
  return rows.length;
...
const stmtGetTeamRatingTotals = db.prepare(`
  SELECT gps.team_id,
    SUM(gps.fg2m + gps.fg3m + gps.fg4m + gps.fg2m_miss + gps.fg3m_miss + gps.fg4m_miss) AS fga,
...
const stmtGetCandidatesForGameType = db.prepare(`
  ...
  SELECT
    ...
    SUM(gps.blk)                                               AS blk,
    SUM(gps.turnover)                                          AS tov,
    SUM(gps.fg2m + gps.fg3m + gps.fg4m)                       AS fgm,
    SUM(gps.fg2m + gps.fg3m + gps.fg4m + gps.fg2m_miss + gps.fg3m_miss + gps.fg4m_miss) AS fga,
    SUM(gps.fg3m)                                              AS fg3m,
    SUM(gps.fg3m + gps.fg3m_miss)                             AS fg3a,
    SUM(gps.fg4m)                                              AS fg4m,
    SUM(gps.fg4m + gps.fg4m_miss)                             AS fg4a,
    SUM(gps.ftm)                                               AS ftm,
    SUM(gps.ftm + gps.ft_miss)                                AS fta,
    SUM(gps.fg2m_miss + gps.fg3m_miss + gps.fg4m_miss)        AS fgmiss,
...
`);
// Head-to-head only ever overrides the point-diff order above for a *two*-team tie on wins —
// with three or more teams tied, a round-robin among them can be circular (A beat B, B beat
// C, C beat A) or incomplete, so that case is left on point-diff/sort_order as already
// ordered by SQL. Win-count ties are already contiguous runs after ORDER BY wins DESC, so
// this only needs one pass grouping equal-wins runs and swapping runs of exactly two.
export function getSeasonStandings(season) {
  const rows = stmtGetSeasonStandings.all(String(season));
  let i = 0;
  while (i < rows.length) {
    let j = i + 1;
    ...
    if (j - i === 2) {
      ...
      const h2h = getHeadToHeadRecordForSeason(a.id, b.id, season);
      if (h2h.teamBWins > h2h.teamAWins) { rows[i] = b; rows[j - 1] = a; }
    }
    i = j;
  }
  return rows;
...
// "Close out" calculator breakdown — set together with price_per_player when a session is
// completed (or ahead of time via savePapawisEstimate), purely for record-keeping/estimating:
// court rate × hours + referee rate × hours, floored at min_per_player. None of these feed
// back into the actual charge; price_per_player alone drives that.
if (!_papawisCols.includes('court_rate_per_hour')) db.exec(`ALTER TABLE papawis_games ADD COLUMN court_rate_per_hour REAL`);
if (!_papawisCols.includes('session_hours'))       db.exec(`ALTER TABLE papawis_games ADD COLUMN session_hours REAL`);
if (!_papawisCols.includes('has_referee'))         db.exec(`ALTER TABLE papawis_games ADD COLUMN has_referee INTEGER NOT NULL DEFAULT 0`);
if (!_papawisCols.includes('actual_total'))        db.exec(`ALTER TABLE papawis_games ADD COLUMN actual_total REAL`);
if (!_papawisCols.includes('min_per_player'))      db.exec(`ALTER TABLE papawis_games ADD COLUMN min_per_player REAL`);
// referee_fee → referee_rate_per_hour rename: the referee is now priced the same way the
// court is (₱/hr × session hours) rather than one flat fee — copies over any value already
// on file under the old (flat-fee) column name before it stops being written to.
if (!_papawisCols.includes('referee_rate_per_hour')) {
  db.exec(`ALTER TABLE papawis_games ADD COLUMN referee_rate_per_hour REAL`);
  if (_papawisCols.includes('referee_fee')) db.exec(`UPDATE papawis_games SET referee_rate_per_hour = referee_fee`);
}
...
// ── Papawis Courts ───────────────────────────────────────────────────────────
// A venue + price reference list — deliberately NOT a foreign key on papawis_games.location
// (that column stays free text, unchanged), so renaming or archiving a court never touches
// past sessions. Rate is per HOUR (not per player) — the "Close out" calculator multiplies
// this by session hours, adds a referee fee if any, then divides by headcount for the actual
// per-player charge.
db.exec(`
  CREATE TABLE IF NOT EXISTS papawis_courts (
    id              TEXT PRIMARY KEY,
    name            TEXT NOT NULL,
...
`);
// price_per_player → price_per_hour rename, for any db file created while this table briefly
// shipped under the old (per-player) pricing model — every row on file at the time had price
// 0 anyway, so a straight copy-and-drop-the-old-name is safe with zero data loss.
{
  const _courtCols = db.prepare('PRAGMA table_info(papawis_courts)').all().map(c => c.name);
  if (!_courtCols.includes('price_per_hour')) {
    db.exec(`ALTER TABLE papawis_courts ADD COLUMN price_per_hour REAL NOT NULL DEFAULT 0`);
    if (_courtCols.includes('price_per_player')) {
      db.exec(`UPDATE papawis_courts SET price_per_hour = price_per_player`);
    }
  }
  // A photo for the venue — same "store the compressed image as a data: URI directly in the
  // column" approach as players.picture_url (see updatePlayerPhoto/sendPlayerPhotoUrl), just
  // reused here via the same serving helper. Empty string = no photo, same convention.
  if (!_courtCols.includes('image_url')) db.exec(`ALTER TABLE papawis_courts ADD COLUMN image_url TEXT NOT NULL DEFAULT ''`);
  // Optional structured address, geocoded in preference to the court's (often unmappable)
  // display name — see papawisMapQuery in server.js. Empty = fall back to name-based lookup,
  // same as before this column existed.
  if (!_courtCols.includes('address')) db.exec(`ALTER TABLE papawis_courts ADD COLUMN address TEXT NOT NULL DEFAULT ''`);
}
const stmtInsertCourt   = db.prepare(`INSERT INTO papawis_courts (id, name, price_per_hour, address, active, sort_order, created_at) VALUES (?, ?, ?, ?, 1, ?, ?)`);
const stmtUpdateCourt   = db.prepare(`UPDATE papawis_courts SET name = ?, price_per_hour = ?, address = ? WHERE id = ?`);
const stmtSetCourtActive = db.prepare(`UPDATE papawis_courts SET active = ? WHERE id = ?`);
const stmtAllCourts     = db.prepare(`SELECT * FROM papawis_courts ORDER BY sort_order ASC, name ASC`);
const stmtActiveCourts  = db.prepare(`SELECT * FROM papawis_courts WHERE active = 1 ORDER BY sort_order ASC, name ASC`);
const stmtCourtByName   = db.prepare(`SELECT * FROM papawis_courts WHERE name = ? AND active = 1 LIMIT 1`);
const stmtCourtById     = db.prepare(`SELECT * FROM papawis_courts WHERE id = ?`);
const stmtMaxCourtSort  = db.prepare(`SELECT COALESCE(MAX(sort_order), -1) AS m FROM papawis_courts`);
const stmtUpdateCourtImage = db.prepare(`UPDATE papawis_courts SET image_url = ? WHERE id = ?`);
const stmtSetCourtSortOrder = db.prepare(`UPDATE papawis_courts SET sort_order = ? WHERE id = ?`);
// orderedIds is the full court list's ids in their new order (drag-drop on the admin Courts
// page) — each just gets its array index as its new sort_order, same "assign 0..n-1 by
// position" approach as the marketplace photo reorder.
const reorderCourtsTx = db.transaction((orderedIds) => {
  orderedIds.forEach((id, i) => stmtSetCourtSortOrder.run(i, id));
});
export function reorderPapawisCourts(orderedIds) { reorderCourtsTx(orderedIds); }

export function addPapawisCourt(name, pricePerHour, address = '') {
  const id = `pc_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  const sortOrder = (stmtMaxCourtSort.get()?.m ?? -1) + 1;
  stmtInsertCourt.run(id, String(name || '').trim(), Number(pricePerHour) || 0, String(address || '').trim(), sortOrder, Date.now());
  return id;
}
export function updatePapawisCourt(id, name, pricePerHour, address = '') {
  stmtUpdateCourt.run(String(name || '').trim(), Number(pricePerHour) || 0, String(address || '').trim(), id);
}
export function setPapawisCourtActive(id, active)  { stmtSetCourtActive.run(active ? 1 : 0, id); }
export function getAllPapawisCourts()               { return stmtAllCourts.all(); }
export function getActivePapawisCourts()            { return stmtActiveCourts.all(); }
export function getPapawisCourtByName(name)         { return stmtCourtByName.get(String(name || '')) || null; }
export function getPapawisCourtById(id)             { return stmtCourtById.get(id) || null; }
export function updatePapawisCourtImage(id, dataUrl) { stmtUpdateCourtImage.run(dataUrl || '', id); }

// One-time seed from the previously-hardcoded location list (views/admin/papawis.js), so the
// dropdown doesn't go blank on the first request after this table exists. Price starts at 0 —
// admin fills those in once from the new Courts page; a 0 price is treated the same as "no
// court match" by the "Close out" default-price lookup, so nothing regresses in the meantime.
{
  if (stmtAllCourts.all().length === 0) {
    const seedNames = [
    ...
    seedNames.forEach((name, i) => stmtInsertCourt.run(`pc_seed_${i}`, name, 0, i, Date.now()));
  }
}

// Geocoded coordinates for a papawis game's free-text location — used to show a default
// map banner on cards with no court photo. Keyed by the normalized location text (not the
// game) so every game at the same venue shares one lookup. Populated lazily by a
// fire-and-forget background call on GET /papawis (see server.js) rather than blocking that
// request on a network call — a location with no cache entry yet just falls back to no
// banner for that one render, same as an unmatched/photo-less court already does.
db.exec(`
  CREATE TABLE IF NOT EXISTS papawis_location_geocode (
...
`);
// The composited dark-map banner image (data: URI) for this location — built once from
// OSM tiles alongside the geocode lookup (see buildPapawisMapImage in server.js) and cached
// here forever, same "store the compressed image as a data: URI directly in the column"
// convention as players.picture_url/papawis_courts.image_url.
if (!db.prepare('PRAGMA table_info(papawis_location_geocode)').all().some(c => c.name === 'map_image')) {
  db.exec(`ALTER TABLE papawis_location_geocode ADD COLUMN map_image TEXT NOT NULL DEFAULT ''`);
}
const stmtGetPapawisLocationGeocode = db.prepare(`SELECT lat, lng, map_image FROM papawis_location_geocode WHERE location_key = ?`);
const stmtSetPapawisLocationGeocode = db.prepare(`
  INSERT INTO papawis_location_geocode (location_key, lat, lng, map_image, geocoded_at) VALUES (?, ?, ?, ?, ?)
  ON CONFLICT(location_key) DO UPDATE SET lat = excluded.lat, lng = excluded.lng, map_image = excluded.map_image, geocoded_at = excluded.geocoded_at
`);
function normalizePapawisLocationKey(text) { return String(text || '').trim().toLowerCase(); }
export function getPapawisLocationGeocode(text) {
  const key = normalizePapawisLocationKey(text);
  return key ? (stmtGetPapawisLocationGeocode.get(key) || null) : null;
}
export function setPapawisLocationGeocode(text, lat, lng, mapImage = '') {
  const key = normalizePapawisLocationKey(text);
  if (!key || !Number.isFinite(lat) || !Number.isFinite(lng)) return;
  stmtSetPapawisLocationGeocode.run(key, lat, lng, mapImage || '', Date.now());
}
...
// Regulars — confirmed (real signup, not a guest slot) in at least minGames of the last
// windowSize non-cancelled papawis games. A rolling recent window rather than an all-time
// count, same reasoning as getMaxPapawisPrice's lookback: someone who played every week for
// a year but stopped months ago shouldn't still read as a "regular" forever.
const stmtFrequentPapawisPlayers = db.prepare(`
  SELECT s.player_id, p.name AS player_name, t.name AS team_name, COUNT(*) AS games_played
  FROM papawis_signups s
  JOIN players p ON p.id = s.player_id
  LEFT JOIN teams t ON t.id = p.team_id
  WHERE s.status = 'confirmed' AND s.guest_name = ''
    AND s.game_id IN (SELECT id FROM papawis_games WHERE status != 'cancelled' ORDER BY date DESC LIMIT ?)
  GROUP BY s.player_id
  HAVING games_played >= ?
  ORDER BY games_played DESC
`);
export function getFrequentPapawisPlayers(windowSize = 10, minGames = 5) {
  return stmtFrequentPapawisPlayers.all(windowSize, minGames);
}
...
const stmtPapawisGames = db.prepare(`
  SELECT g.*,
    ...
    (SELECT COUNT(*) FROM papawis_signups s WHERE s.game_id = g.id AND s.status IN ('waitlist', 'pending'))  AS waitlist_count
...
const stmtSetPapawisGameCompleted = db.prepare(`
  UPDATE papawis_games SET status = 'completed', price_per_player = ?, completed_at = ?,
    court_rate_per_hour = ?, session_hours = ?, has_referee = ?, referee_rate_per_hour = ?,
    actual_total = ?, min_per_player = ?
  WHERE id = ?
`);
...
// 'pending' = a probationary player's signup, held for admin deposit confirmation before
// it enters the normal confirmed/waitlist flow — see joinPapawisGame() and
// promotePapawisPendingSignup(). Sorted last so it reads as "still waiting" everywhere
// this feeds (admin board, public roster).
const stmtPapawisSignups = db.prepare(`
  SELECT s.*, p.name AS player_name, p.team_id, t.name AS team_name
  FROM papawis_signups s
  JOIN players p ON p.id = s.player_id
  LEFT JOIN teams t ON t.id = p.team_id
  WHERE s.game_id = ? AND s.status IN ('confirmed', 'waitlist', 'pending')
  ORDER BY (CASE s.status WHEN 'confirmed' THEN 0 WHEN 'waitlist' THEN 1 ELSE 2 END), s.sort_order ASC, s.signed_up_at ASC
`);
...
const stmtPapawisActiveSignupForPlayer = db.prepare(`
  SELECT * FROM papawis_signups WHERE game_id = ? AND player_id = ? AND guest_name = '' AND status IN ('confirmed', 'waitlist', 'pending')
`);
...
// Highest price_per_player among the last `lookback` closed-out sessions (most recent
// game date first) — used as the probation deposit floor. A rolling recent window rather
// than all-time: one old outlier (pricier court, a referee that's no longer used) would
// otherwise keep the floor stuck high forever even after prices settle back down. Null
// until at least one game has been completed.
const stmtRecentPapawisPrices = db.prepare(`
  SELECT price_per_player FROM papawis_games
  WHERE status = 'completed' AND price_per_player IS NOT NULL
  ORDER BY date DESC, completed_at DESC
  LIMIT ?
`);
export function getMaxPapawisPrice(lookback = 5) {
  const rows = stmtRecentPapawisPrices.all(lookback);
  return rows.length ? Math.max(...rows.map(r => r.price_per_player)) : null;
}
...
// Every unpaid confirmed slot across every completed game, in one query — the admin-facing
// "who still owes money" view (views/admin/papawis.js's list page), so an admin doesn't have
// to open each completed game individually to find them. Same underlying signal as
// hasUnpaidCompletedPapawis() above (confirmed + game completed + paid_at IS NULL), just
// listing every matching row instead of a single yes/no for one player.
const stmtAllUnpaidCompletedPapawisSignups = db.prepare(`
  SELECT s.*, p.name AS player_name, p.team_id, t.name AS team_name,
    g.title AS game_title, g.date AS game_date, g.location AS game_location, g.price_per_player
  FROM papawis_signups s
  JOIN papawis_games g ON g.id = s.game_id
  JOIN players p ON p.id = s.player_id
  LEFT JOIN teams t ON t.id = p.team_id
  WHERE s.status = 'confirmed' AND g.status = 'completed' AND s.paid_at IS NULL
  ORDER BY p.name ASC, g.date DESC
`);
export function getAllUnpaidCompletedPapawisSignups() { return stmtAllUnpaidCompletedPapawisSignups.all(); }
...
// The other half of "one payment, two games": a payment already linked to some signup can
// still be reused for a second one — e.g. two games back-to-back, paid in one lump sum on
// the second game's day. Joined with the game it's already linked to, so the UI can label
// it ("already used for <that game>") rather than presenting it as a fresh, unexplained
// match. The caller (server.js) is what actually enforces that the combined price of every
// game a payment covers adds up to the payment's amount — this query just surfaces candidates.
const stmtReusablePapawisPayments = db.prepare(`
  SELECT tl.*, ps.game_id AS linked_game_id, g.title AS linked_game_title, g.date AS linked_game_date
  FROM transaction_ledger tl
  JOIN papawis_signups ps ON ps.paid_tx_id = tl.id
  JOIN papawis_games g ON g.id = ps.game_id
  WHERE tl.player_id = ? AND tl.type = 'payment' AND tl.status = 'confirmed' AND LOWER(tl.category) = 'papawis'
  ORDER BY tl.date DESC, tl.created_at DESC
`);
export function getReusablePapawisPayments(playerId) { return stmtReusablePapawisPayments.all(playerId); }

const stmtPapawisSignupsByTxId = db.prepare(`SELECT * FROM papawis_signups WHERE paid_tx_id = ?`);
export function getPapawisSignupsByTxId(txId) { return txId ? stmtPapawisSignupsByTxId.all(txId) : []; }
...
// Every held signup for a player, across any game — used to promote them all once a
// probation deposit is confirmed, regardless of whether that confirmation happened through
// the Papawis admin page (one signup at a time) or the generic ledger confirm route (which
// only knows the player, not which game they're waiting on).
const stmtPendingPapawisSignupsForPlayer = db.prepare(`SELECT * FROM papawis_signups WHERE player_id = ? AND status = 'pending'`);
export function getPendingPapawisSignupsForPlayer(playerId) { return stmtPendingPapawisSignupsForPlayer.all(playerId); }

// A player-submitted (settle-balance) deposit still awaiting admin confirmation — surfaced
// on the Papawis admin page as a "possible match" so confirming it reuses that exact
// transaction instead of recording a second, duplicate payment.
const stmtUnconfirmedPapawisDeposit = db.prepare(`
  SELECT * FROM transaction_ledger
  WHERE player_id = ? AND type = 'payment' AND status = 'pending' AND category = 'Papawis Deposit'
  ORDER BY created_at DESC LIMIT 1
`);
export function getUnconfirmedPapawisDeposit(playerId) { return stmtUnconfirmedPapawisDeposit.get(playerId) || null; }
...
// isProbation: bypasses the normal confirmed/waitlist capacity check entirely — the
// player lands in 'pending' regardless of open slots, and only an admin confirming their
// deposit (see promotePapawisPendingSignup) moves them into the real confirmed/waitlist
// flow. See players.papawis_probation.
export const joinPapawisGame = db.transaction((gameId, playerId, signupId, isProbation = false) => {
  ...
  if (isProbation) {
    stmtInsertPapawisSignup.run(signupId, gameId, playerId, 'pending', Date.now(), '', nextPapawisSortOrder(gameId, 'pending'));
    logPapawisActivity({ gameId, eventType: 'joined', playerId, playerName, notes: 'pending deposit' });
...
// Admin confirms a probationary player's deposit — moves their held 'pending' signup into
// the real confirmed/waitlist flow, using the same capacity check joinPapawisGame would
// have applied at join time (a slot may have opened or filled since they first listed).
export const promotePapawisPendingSignup = db.transaction((signupId) => {
  const signup = stmtPapawisSignupById.get(signupId);
  if (!signup || signup.status !== 'pending') return { error: 'not_found' };
  const confirmedCount = stmtCountConfirmed.get(signup.game_id).n;
  const game = stmtPapawisGame.get(signup.game_id);
  const status = confirmedCount < (game?.max_slots ?? 0) ? 'confirmed' : 'waitlist';
  stmtSetPapawisSignupStatus.run(status, nextPapawisSortOrder(signup.game_id, status), signupId);
  const playerName = stmtPlayerNameById.get(signup.player_id)?.name || '';
  logPapawisActivity({ gameId: signup.game_id, eventType: 'joined', playerId: signup.player_id, playerName, notes: `deposit confirmed — ${status}` });
  return { status };
});
...
export function completePapawisGame(gameId, pricePerPlayer, breakdown = {}) {
  const { courtRate = null, hours = null, hasReferee = false, refereeRate = null, actualTotal = null, minPerPlayer = null } = breakdown;
  stmtSetPapawisGameCompleted.run(
    pricePerPlayer, Date.now(),
    courtRate, hours, hasReferee ? 1 : 0, refereeRate, actualTotal, minPerPlayer,
    gameId
  );
  return getPapawisGame(gameId);
}

const stmtSavePapawisEstimate = db.prepare(`
  UPDATE papawis_games SET
    court_rate_per_hour = ?, session_hours = ?, has_referee = ?, referee_rate_per_hour = ?, actual_total = ?, min_per_player = ?
  WHERE id = ? AND status = 'open'
`);
// Same breakdown columns completePapawisGame writes, but savable ahead of the game actually
// happening (while status is still 'open') and without touching price_per_player/completed_at
// or charging anyone. This is what lets the pre-game reminder email quote a real estimate
// instead of the generic headcount-only guess (see defaultPapawisPrice) — an admin who's
// already booked the court and referee can save that total the moment they know it.
export function savePapawisEstimate(gameId, breakdown = {}) {
  const { courtRate = null, hours = null, hasReferee = false, refereeRate = null, actualTotal = null, minPerPlayer = null } = breakdown;
  stmtSavePapawisEstimate.run(courtRate, hours, hasReferee ? 1 : 0, refereeRate, actualTotal, minPerPlayer, gameId);
  return getPapawisGame(gameId);
}
export function cancelPapawisGame(gameId) { stmtSetPapawisGameCancelled.run(gameId); return getPapawisGame(gameId); }

// Free-text, same as at creation — not a foreign key on papawis_courts, so this works
// whether the admin picks a known court or types something else entirely.
const stmtSetPapawisGameLocation = db.prepare(`UPDATE papawis_games SET location = ? WHERE id = ?`);
export function setPapawisGameLocation(gameId, location) { stmtSetPapawisGameLocation.run(String(location || '').trim(), gameId); }

const stmtSetPapawisGameTime = db.prepare(`UPDATE papawis_games SET start_time = ?, end_time = ? WHERE id = ?`);
export function setPapawisGameTime(gameId, startTime, endTime) {
  stmtSetPapawisGameTime.run(String(startTime || '').trim(), String(endTime || '').trim(), gameId);
}

// Lowering this below the current confirmed count doesn't bump anyone off — confirmed stays
// confirmed either way, same as the site never re-checks capacity except at join time
// (joinPapawisGame/promotePapawisPendingSignup) — this only changes the cap those checks use.
const stmtSetPapawisGameMaxSlots = db.prepare(`UPDATE papawis_games SET max_slots = ? WHERE id = ?`);
export function setPapawisGameMaxSlots(gameId, maxSlots) {
  stmtSetPapawisGameMaxSlots.run(Math.max(1, Number(maxSlots) || 1), gameId);
}
...
// Active roster for a head's own-team page — inactive players (departed, etc.) excluded
// same as the rest of the public site's player listings.
const stmtPlayersByTeam = db.prepare(`SELECT * FROM players WHERE team_id = ? AND status != 'inactive' ORDER BY sort_order ASC, name ASC`);
export function getPlayersByTeam(teamId) { return stmtPlayersByTeam.all(teamId); }

// ── League Polls ─────────────────────────────────────────────────────────────
// Single-question, multiple-choice, admin-created polls with two independent tier settings:
// `visibility` (who can see the poll/results) and `voter_eligibility` (who's actually allowed
// to vote) — a poll can be visible wider than it's votable (e.g. shown to everyone, only heads
// vote). Tiers are 'admins' | 'heads' | 'players', cascading — an admin can see/vote anything
// a head can, a head anything a player can — resolved in server.js's currentPollVoterIdentity/
// pollTierQualifies, not stored here; this file just persists the raw rows.
db.exec(`
  CREATE TABLE IF NOT EXISTS league_polls (
    ...
    voter_eligibility TEXT NOT NULL DEFAULT 'players',
    created_by        TEXT NOT NULL DEFAULT '',
    created_at        INTEGER NOT NULL
  )
`);
{
  const _pollCols = db.prepare('PRAGMA table_info(league_polls)').all().map(c => c.name);
  if (!_pollCols.includes('description')) db.exec(`ALTER TABLE league_polls ADD COLUMN description TEXT NOT NULL DEFAULT ''`);
}
db.exec(`
...
`);
db.exec(`CREATE INDEX IF NOT EXISTS idx_league_poll_votes_poll ON league_poll_votes(poll_id)`);

const stmtInsertPoll   = db.prepare(`
  INSERT INTO league_polls (id, question, description, options, status, visibility, voter_eligibility, created_by, created_at)
  VALUES (?, ?, ?, ?, 'open', ?, ?, ?, ?)
`);
const stmtAllPolls     = db.prepare(`SELECT * FROM league_polls ORDER BY created_at DESC`);
const stmtPollById     = db.prepare(`SELECT * FROM league_polls WHERE id = ?`);
const stmtSetPollStatus = db.prepare(`UPDATE league_polls SET status = ? WHERE id = ?`);
const stmtPollVotes    = db.prepare(`SELECT * FROM league_poll_votes WHERE poll_id = ? ORDER BY created_at ASC`);
const stmtMyPollVote   = db.prepare(`SELECT * FROM league_poll_votes WHERE poll_id = ? AND voter_id = ?`);
const stmtCastVote     = db.prepare(`
  INSERT INTO league_poll_votes (id, poll_id, voter_id, voter_name, option_index, created_at)
  VALUES (?, ?, ?, ?, ?, ?)
  ON CONFLICT(poll_id, voter_id) DO UPDATE SET option_index = excluded.option_index, voter_name = excluded.voter_name, created_at = excluded.created_at
`);

export function createLeaguePoll({ question, description = '', options, visibility = 'players', voterEligibility = 'players', createdBy = '' }) {
  const id = `poll_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  stmtInsertPoll.run(id, question.trim(), (description || '').trim(), JSON.stringify(options), visibility, voterEligibility, createdBy, Date.now());
  return id;
}
export function getAllLeaguePolls()            { return stmtAllPolls.all().map(p => ({ ...p, options: JSON.parse(p.options) })); }
export function getLeaguePollById(id) {
  const p = stmtPollById.get(id);
  return p ? { ...p, options: JSON.parse(p.options) } : null;
}
export function setLeaguePollStatus(id, status) { stmtSetPollStatus.run(status, id); }
export function getLeaguePollVotes(pollId)      { return stmtPollVotes.all(pollId); }
export function getMyLeaguePollVote(pollId, voterId) { return stmtMyPollVote.get(pollId, voterId) || null; }
// Upsert keyed on (poll_id, voter_id) — casting again just changes the existing vote, same
// "you can change it below" behavior as fine-case voting.
export function castLeaguePollVote(pollId, voterId, voterName, optionIndex) {
  const existing = stmtMyPollVote.get(pollId, voterId);
  const id = existing?.id || `plv_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  stmtCastVote.run(id, pollId, voterId, voterName, optionIndex, Date.now());
}
...
// points + min_suspension_games added for the league's conduct-points policy — ALTERed
// here (columns must exist before any INSERT/SELECT against them below), but the actual
// one-time backfill of values runs further down, after createFineCategory is defined.
const _fcJustAddedPoints = (() => {
  const cols = db.prepare('PRAGMA table_info(fine_categories)').all().map(c => c.name);
  const justAdded = !cols.includes('points');
  if (justAdded) db.exec(`ALTER TABLE fine_categories ADD COLUMN points INTEGER NOT NULL DEFAULT 0`);
  if (!cols.includes('min_suspension_games')) db.exec(`ALTER TABLE fine_categories ADD COLUMN min_suspension_games INTEGER NOT NULL DEFAULT 0`);
  return justAdded;
})();
...
const stmtInsertFineCategory = db.prepare(`
  INSERT INTO fine_categories (id, label, amount, description, examples, sort_order, points, min_suspension_games) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
`);
export function createFineCategory({ label, amount, description = '', examples = [], points = 0, minSuspensionGames = 0 }) {
  ...
  stmtInsertFineCategory.run(id, label, amount, description, JSON.stringify(examples || []), maxOrder + 1, points, minSuspensionGames);
  return id;
}
const stmtUpdateFineCategory = db.prepare(`UPDATE fine_categories SET label = ?, amount = ?, description = ?, examples = ?, points = ?, min_suspension_games = ? WHERE id = ?`);
export function updateFineCategory(id, { label, amount, description = '', examples = [], points = 0, minSuspensionGames = 0 }) {
  stmtUpdateFineCategory.run(label, amount, description, JSON.stringify(examples || []), points, minSuspensionGames, id);
}
...
// One-time backfill for the conduct-points policy: assigns point values to the two
// categories that predate the `points` column and seeds the two new tiers the updated
// policy added (Minor/Admin, Physical Altercation — the latter carries a mandatory
// suspension floor independent of accumulated points). Guarded by _fcJustAddedPoints
// (set above, at ALTER time) so this runs exactly once, ever — an admin's later edits to
// any category, including these two, are never overwritten by a future deploy.
if (_fcJustAddedPoints) {
  db.prepare(`UPDATE fine_categories SET points = 2 WHERE label = 'Standard Unsportsmanlike Conduct'`).run();
  db.prepare(`UPDATE fine_categories SET points = 3 WHERE label = 'Serious or Repeated Violations'`).run();
  if (!db.prepare(`SELECT 1 FROM fine_categories WHERE label = 'Minor / Admin'`).get()) {
    createFineCategory({
      label: 'Minor / Admin', amount: 200,
      description: 'Small, low-severity administrative or conduct issues.',
      examples: ['Late arrival without notice', 'Uniform/jersey violation', 'Minor delay-of-game'],
      points: 1,
    });
  }
  if (!db.prepare(`SELECT 1 FROM fine_categories WHERE label = 'Physical Altercation'`).get()) {
    createFineCategory({
      label: 'Physical Altercation', amount: 2200,
      description: 'Actual physical contact in anger, or fighting — carries a mandatory minimum suspension regardless of accumulated points.',
      examples: ['Throwing a punch or shoving in anger', 'Fighting on or off the court'],
      points: 4, minSuspensionGames: 2,
    });
  }
}
...
// Whether the player has actually settled an approved fine — the shared ledger only tracks
// a player's overall running balance, not which specific charge a later payment covers (a
// player with a fine AND an unrelated season-fee balance can't be disambiguated from
// current_balance alone). Same fix Papawis used for the identical problem: an explicit
// admin-set marker, paid_tx_id linking to the exact payment transaction it created so
// un-marking can void that transaction precisely instead of guessing which one to reverse.
const _fineCaseCols = db.prepare('PRAGMA table_info(fine_cases)').all().map(c => c.name);
if (!_fineCaseCols.includes('paid_at'))    db.exec(`ALTER TABLE fine_cases ADD COLUMN paid_at INTEGER`);
if (!_fineCaseCols.includes('paid_tx_id')) db.exec(`ALTER TABLE fine_cases ADD COLUMN paid_tx_id TEXT NOT NULL DEFAULT ''`);

// points/min_suspension_games are snapshotted onto the case at creation time, same as
// amount already is — so a later edit to a category's point value never rewrites the
// history of what applied when a past case was actually charged. season is stamped from
// getPortalCurrentSeason() at creation so active-points math (getPlayerSuspensionStatus)
// can scope "active" to the current season, per the league's per-season reset policy.
// Cases created before these columns existed backfill once below: points/suspension floor
// from whatever their category currently holds, season set to the current portal season
// since every case so far was filed within it.
const _fineCaseJustAddedPoints = !_fineCaseCols.includes('points');
if (_fineCaseJustAddedPoints) db.exec(`ALTER TABLE fine_cases ADD COLUMN points INTEGER NOT NULL DEFAULT 0`);
if (!_fineCaseCols.includes('min_suspension_games')) db.exec(`ALTER TABLE fine_cases ADD COLUMN min_suspension_games INTEGER NOT NULL DEFAULT 0`);
if (!_fineCaseCols.includes('season')) db.exec(`ALTER TABLE fine_cases ADD COLUMN season TEXT NOT NULL DEFAULT ''`);
if (_fineCaseJustAddedPoints) {
  const currentSeason = getPortalCurrentSeason();
  for (const kase of db.prepare(`SELECT id, category_id FROM fine_cases`).all()) {
    const cat = kase.category_id ? db.prepare('SELECT points, min_suspension_games FROM fine_categories WHERE id = ?').get(kase.category_id) : null;
    db.prepare(`UPDATE fine_cases SET points = ?, min_suspension_games = ?, season = ? WHERE id = ?`)
      .run(cat?.points || 0, cat?.min_suspension_games || 0, currentSeason, kase.id);
  }
}
...
const stmtInsertFineCase = db.prepare(`
  INSERT INTO fine_cases (id, player_id, player_name, game_id, category_id, category_label, amount, points, min_suspension_games, season, description, reported_by_type, reported_by_id, reported_by_name, status, created_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
...
// sits behind the admin-escalation gate before any head ever sees it. points/
// min_suspension_games/season are snapshotted from the category + current portal season
// at report time, same reasoning as amount already being snapshotted (see the ALTER
// block above) — a later category edit never rewrites what a past case was actually
// charged/counted as.
export function createFineCase({ playerId, gameId = '', categoryId, description, reportedByType, reportedById, reportedByName, status = 'open' }) {
  ...
  stmtInsertFineCase.run(id, playerId, player.name, gameId, categoryId, category.label, category.amount, category.points || 0, category.min_suspension_games || 0, getPortalCurrentSeason(), description, reportedByType, reportedById, reportedByName, status, Date.now());
  return { ok: true, id };
}
...
const stmtMarkFineCasePaid   = db.prepare(`UPDATE fine_cases SET paid_at = ?, paid_tx_id = ? WHERE id = ?`);
const stmtMarkFineCaseUnpaid = db.prepare(`UPDATE fine_cases SET paid_at = NULL, paid_tx_id = '' WHERE id = ?`);
export function markFineCasePaid(id, txId = '') { stmtMarkFineCasePaid.run(Date.now(), txId, id); }
export function markFineCaseUnpaid(id)          { stmtMarkFineCaseUnpaid.run(id); }

// Collection totals across every approved (i.e. actually charged) fine — outstanding is
// simply fined minus paid, since a rejected/dismissed/still-open case never charged anyone.
const stmtFineCollectionRows = db.prepare(`SELECT amount, paid_at FROM fine_cases WHERE status = 'approved'`);
export function getFineCollectionSummary() {
  const rows = stmtFineCollectionRows.all();
  const totalFined = rows.reduce((sum, r) => sum + Number(r.amount || 0), 0);
  const paidRows   = rows.filter(r => r.paid_at != null);
  const totalPaid  = paidRows.reduce((sum, r) => sum + Number(r.amount || 0), 0);
  return {
    caseCount: rows.length,
    paidCount: paidRows.length,
    totalFined,
    totalPaid,
    totalOutstanding: totalFined - totalPaid,
  };
}

// ── Conduct points / suspension tiers ────────────────────────────────────────────
// Maps a player's cumulative *active* conduct points (approved fines only, scoped to a
// single season — see getPlayerSuspensionStatus) to the league's suspension tier. 10+ is
// "League Review" rather than a fixed game count — deliberately open-ended, since the
// league treats that volume of points as needing a human decision, not an auto-suspension.
export function getSuspensionTierForPoints(points) {
  const p = Number(points) || 0;
  if (p >= 10) return { games: null, label: 'League Review', needsReview: true };
  if (p >= 8)  return { games: 3, label: '3 games', needsReview: false };
  if (p >= 6)  return { games: 2, label: '2 games', needsReview: false };
  if (p >= 4)  return { games: 1, label: '1 game', needsReview: false };
  return { games: 0, label: 'None', needsReview: false };
}

// Informational only — nothing in the app currently blocks a suspended player from
// signing up/playing on this alone; it surfaces the tier for an admin to act on manually,
// same "admin has final say" spirit as the rest of this feature. Active points reset each
// season (the league's chosen policy) rather than rolling or decaying, so this only sums
// approved cases whose stamped `season` matches the one asked for. A category's own
// min_suspension_games (e.g. Physical Altercation's mandatory 2-game floor) applies
// per-incident regardless of point total, so the final figure is whichever is higher.
const stmtApprovedCasesForPlayerSeason = db.prepare(`
  SELECT points, min_suspension_games FROM fine_cases WHERE player_id = ? AND season = ? AND status = 'approved'
`);
export function getPlayerSuspensionStatus(playerId, season) {
  const rows = stmtApprovedCasesForPlayerSeason.all(playerId, String(season));
  const activePoints  = rows.reduce((sum, r) => sum + Number(r.points || 0), 0);
  const categoryFloor = rows.reduce((max, r) => Math.max(max, Number(r.min_suspension_games || 0)), 0);
  const tier = getSuspensionTierForPoints(activePoints);
  const gamesSuspended = tier.games === null ? null : Math.max(tier.games, categoryFloor);
  return { activePoints, categoryFloor, tier, gamesSuspended, needsReview: tier.needsReview };
}
...
// Admin-only, cross-player view of the feature — real identities always (the
// anon_alias masking is a display-time thing for other players/the ratee, not
// something baked into the stored row, so admin just reads rater/ratee names
// straight off the players table regardless of is_anonymous).
const stmtAllPeerRatingsForSeason = db.prepare(`
  SELECT pr.*, rater.name AS rater_name, ratee.name AS ratee_name
  FROM peer_ratings pr
  JOIN players rater ON rater.id = pr.rater_player_id
  JOIN players ratee ON ratee.id = pr.ratee_player_id
  WHERE pr.season = ?
  ORDER BY pr.updated_at DESC
`);
export function getAllPeerRatings(season) {
  return stmtAllPeerRatingsForSeason.all(String(season));
}
const stmtPeerRatingSeasons = db.prepare(`SELECT DISTINCT season FROM peer_ratings ORDER BY season DESC`);
export function getPeerRatingSeasons() {
  return stmtPeerRatingSeasons.all().map(r => r.season);
}
