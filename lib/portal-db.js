...
const stmtUpdateMarketplaceListing = db.prepare(`
  ...
  SET title = ?, description = ?, price = ?, compare_at_price = ?, min_buyers = ?, variant_options = ?, updated_at = ?
  WHERE id = ?
`);
export function updateMarketplaceListing(id, { title, description = '', price = 0, compareAtPrice = 0, minBuyers = 0, variantOptions = [] }) {
  stmtUpdateMarketplaceListing.run(title, description, Number(price) || 0, Number(compareAtPrice) || 0, Number(minBuyers) || 0, JSON.stringify(variantOptions), Date.now(), id);
}
