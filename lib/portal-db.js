...
const stmtSetCompareCache = db.prepare(
  `INSERT OR REPLACE INTO compare_cache (pair_key, stats_key, writeup, created_at)
   VALUES (?, ?, ?, ?)`
);

export function getCompareCache(idA, idB, tA, tB) {
  const pair = [idA, idB].sort().join('|');
  const stats = `${tA?.games_played|0}_${tA?.pts|0}_${tA?.reb|0}_${tA?.ast|0}_${tB?.games_played|0}_${tB?.pts|0}_${tB?.reb|0}_${tB?.ast|0}`;
  const row = stmtGetCompareCache.get(pair, stats);
  return row ? row.writeup : null;
}

export function setCompareCache(idA, idB, tA, tB, writeup) {
  const pair = [idA, idB].sort().join('|');
  const stats = `${tA?.games_played|0}_${tA?.pts|0}_${tA?.reb|0}_${tA?.ast|0}_${tB?.games_played|0}_${tB?.pts|0}_${tB?.reb|0}_${tB?.ast|0}`;
  stmtSetCompareCache.run(pair, stats, writeup, Date.now());
}
