...
const stmtInsertSeasonSignup = db.prepare(`
  INSERT OR IGNORE INTO season_signups (id, reg_id, season, status, has_balance, balance_amt, notes, jersey_top, jersey_shorts, quota_ack, created_at)
  VALUES (?, ?, ?, 'waitlisted', ?, ?, '', ?, ?, ?, ?)
`);
const stmtGetSeasonSignup         = db.prepare('SELECT * FROM season_signups WHERE reg_id = ? AND season = ?');
...
export function insertSeasonSignup(regId, season, hasBalance, balanceAmt, jerseyTop = '', jerseyShorts = '', quotaAck = 0) {
  const id = `ss_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  stmtInsertSeasonSignup.run(id, regId, String(season), hasBalance ? 1 : 0, balanceAmt ?? 0, jerseyTop, jerseyShorts, quotaAck ? 1 : 0, Date.now());
...
const stmtInsertReg = db.prepare(`
  INSERT INTO registrations (
    ...
    referred_by, emergency_name, emergency_phone, motto, user_id, gender
  ) VALUES (?, ?, ?, ?, '', 'pending', '', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '', ?)
`);
...
db.exec(`
  CREATE TABLE IF NOT EXISTS award_photo_overrides (
    ...
    updated_at INTEGER NOT NULL DEFAULT 0,
    PRIMARY KEY (season, award_type, player_id)
  )
`);
...
const stmtGetAwardPhotoOverrides = db.prepare(
  'SELECT player_id, offset_x, offset_y, zoom, photo_url FROM award_photo_overrides WHERE season = ? AND award_type = ?'
);
const stmtUpsertAwardPhotoOverride = db.prepare(`
  INSERT INTO award_photo_overrides (season, award_type, player_id, offset_x, offset_y, zoom, photo_url, updated_at)
  VALUES (@season, @award_type, @player_id, @offset_x, @offset_y, @zoom, @photo_url, @updated_at)
  ON CONFLICT(season, award_type, player_id) DO UPDATE SET
...
const stmtDeleteAwardPhotoOverride = db.prepare(
  'DELETE FROM award_photo_overrides WHERE season = ? AND award_type = ? AND player_id = ?'
);
...
export function upsertAwardPhotoOverride({ season, award_type, player_id, offset_x = 50, offset_y = 50, zoom = 1, photo_url = '' }) {
  stmtUpsertAwardPhotoOverride.run({ season, award_type, player_id, offset_x, offset_y, zoom, photo_url, updated_at: Date.now() });
}
export function deleteAwardPhotoOverride(season, award_type, player_id) {
  stmtDeleteAwardPhotoOverride.run(season, award_type, player_id);
}
...
export function getMvpCandidates(season) {
  ...
  return stmtGetMvpCandidates.all(s, s, s);
}
