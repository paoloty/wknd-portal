...
// past sessions. Exists purely to (a) back the location dropdown instead of the old hardcoded
// PAPAWIS_LOCATIONS array, and (b) let the "Close out" price default be looked up by name
// match instead of the generic ₱4,200-budget heuristic (see defaultPapawisPrice in
// views/admin/papawis.js) — a real per-court rate beats one guess applied to every venue.
db.exec(`
...
`);
const stmtInsertCourt   = db.prepare(`INSERT INTO papawis_courts (id, name, price_per_player, active, sort_order, created_at) VALUES (?, ?, ?, 1, ?, ?)`);
const stmtUpdateCourt   = db.prepare(`UPDATE papawis_courts SET name = ?, price_per_player = ? WHERE id = ?`);
...
export function addPapawisCourt(name, pricePerPlayer) {
  const id = `pc_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  const sortOrder = (stmtMaxCourtSort.get()?.m ?? -1) + 1;
  stmtInsertCourt.run(id, String(name || '').trim(), Number(pricePerPlayer) || 0, sortOrder, Date.now());
  return id;
}
export function updatePapawisCourt(id, name, pricePerPlayer) {
  stmtUpdateCourt.run(String(name || '').trim(), Number(pricePerPlayer) || 0, id);
}
