...
const stmtInsertReg = db.prepare(`
  INSERT INTO registrations (
    ...
    referred_by, emergency_name, emergency_phone, motto, user_id
  ) VALUES (?, ?, ?, ?, '', 'pending', '', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '')
`);
...
const stmtRegByEmail   = db.prepare('SELECT * FROM registrations WHERE email = ? ORDER BY created_at DESC LIMIT 1');
...
export function insertRegistration({
  ...
  referred_by = '', emergency_name = '', emergency_phone = '', motto = '',
}) {
  stmtInsertReg.run(
...
const stmtCreateGame = db.prepare(`
  INSERT INTO games (id, date, team_a_id, team_b_id, team_a_name, team_b_name,
    team_a_score, team_b_score, season, game_type, series_id, scheduled, sort_order)
  VALUES (?, ?, ?, ?, ?, ?, 0, 0, ?, ?, ?, 1, 0)
`);
