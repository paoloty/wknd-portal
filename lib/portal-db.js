...
const stmtGetSeasonAwards = db.prepare(`
  SELECT a.id, a.award_type, a.season, a.notes, a.player_id, a.team_id,
         ...
         t.name AS team_name, t.color AS team_color,
         pt.pts, pt.ast, pt.reb, pt.stl, pt.blk, pt.games_played,
         pt.fg2m, pt.fg3m, pt.fg2m_miss, pt.fg3m_miss, pt.ftm, pt.ft_miss
  FROM awards a
  LEFT JOIN players p ON p.id = a.player_id
  LEFT JOIN teams t ON t.id = p.team_id
  LEFT JOIN player_totals pt ON pt.player_id = a.player_id
  WHERE a.season = ?
  ORDER BY a.id ASC
`);
