...
// completed, purely for record-keeping (so an admin can look back and see how a price was
// derived: court rate × hours + referee fee, floored at min_per_player). None of these feed
// back into anything else; price_per_player alone still drives the actual per-player charge.
...
if (!_papawisCols.includes('referee_fee'))         db.exec(`ALTER TABLE papawis_games ADD COLUMN referee_fee REAL`);
...
const stmtSetPapawisGameCompleted = db.prepare(`
  UPDATE papawis_games SET status = 'completed', price_per_player = ?, completed_at = ?,
    court_rate_per_hour = ?, session_hours = ?, has_referee = ?, referee_fee = ?,
...
export function completePapawisGame(gameId, pricePerPlayer, breakdown = {}) {
  const { courtRate = null, hours = null, hasReferee = false, refereeFee = null, actualTotal = null, minPerPlayer = null } = breakdown;
  stmtSetPapawisGameCompleted.run(
    pricePerPlayer, Date.now(),
    courtRate, hours, hasReferee ? 1 : 0, refereeFee, actualTotal, minPerPlayer,
    gameId
  );
