...
const stmtInsertCourt   = db.prepare(`INSERT INTO papawis_courts (id, name, price_per_hour, active, sort_order, created_at) VALUES (?, ?, ?, 1, ?, ?)`);
const stmtUpdateCourt   = db.prepare(`UPDATE papawis_courts SET name = ?, price_per_hour = ? WHERE id = ?`);
...
export function addPapawisCourt(name, pricePerHour) {
  const id = `pc_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  const sortOrder = (stmtMaxCourtSort.get()?.m ?? -1) + 1;
  stmtInsertCourt.run(id, String(name || '').trim(), Number(pricePerHour) || 0, sortOrder, Date.now());
  return id;
}
export function updatePapawisCourt(id, name, pricePerHour) {
  stmtUpdateCourt.run(String(name || '').trim(), Number(pricePerHour) || 0, id);
}
