...
const stmtInsertSeasonSignup = db.prepare(`
  INSERT OR IGNORE INTO season_signups (id, reg_id, season, status, has_balance, balance_amt, notes, jersey_top, jersey_shorts, quota_ack, team_pref, created_at)
  VALUES (?, ?, ?, 'waitlisted', ?, ?, '', ?, ?, ?, ?, ?)
`);
...
export function insertSeasonSignup(regId, season, hasBalance, balanceAmt, jerseyTop = '', jerseyShorts = '', quotaAck = 0, teamPref = '') {
  const id = `ss_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  stmtInsertSeasonSignup.run(id, regId, String(season), hasBalance ? 1 : 0, balanceAmt ?? 0, jerseyTop, jerseyShorts, quotaAck ? 1 : 0, teamPref, Date.now());
...
const stmtPapawisSignups = db.prepare(`
  ...
  ORDER BY (s.status = 'waitlist'), s.signed_up_at ASC
`);
...
const stmtInsertPapawisSignup = db.prepare(`
  INSERT INTO papawis_signups (id, game_id, player_id, status, signed_up_at, guest_name)
  VALUES (?, ?, ?, ?, ?, ?)
`);
const stmtCancelPapawisSignup = db.prepare(`
  UPDATE papawis_signups SET status = 'cancelled', cancelled_at = ? WHERE id = ?
`);
const stmtPromotePapawisSignup = db.prepare(`
  UPDATE papawis_signups SET status = 'confirmed' WHERE id = ?
`);
...
// never delayed (open_days_before is null) or the game date is within that many days.
...
export const joinPapawisGame = db.transaction((gameId, playerId, signupId) => {
  ...
  stmtInsertPapawisSignup.run(signupId, gameId, playerId, status, Date.now(), '');
...
export const cancelPapawisSignup = db.transaction((signupId, actor = 'player') => {
  ...
  if (wasConfirmed) {
    ...
    if (next) {
      stmtPromotePapawisSignup.run(next.id);
      promoted = next;
    }
  }
...
export function adminAddPapawisSignup({ id, gameId, playerId, status = 'confirmed', guestName = '' }) {
  ...
  stmtInsertPapawisSignup.run(id, gameId, playerId, status, Date.now(), guestName || '');
