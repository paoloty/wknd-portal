...
const stmtInsertSeasonSignup = db.prepare(`
  INSERT OR IGNORE INTO season_signups
    (id, reg_id, season, status, has_balance, balance_amt, notes,
     ...
     pockets, reshuffle_vote, payment_plan, comments, contact_changed_at, contact_change_note, created_at)
  VALUES (?, ?, ?, 'waitlisted', ?, ?, '', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
...
export function insertSeasonSignup(
  regId, season, hasBalance, balanceAmt, jerseyTop = '', jerseyShorts = '', quotaAck = 0, teamPref = '',
  {
    ...
    contactChangedAt = 0, contactChangeNote = '',
  } = {}
) {
  ...
  stmtInsertSeasonSignup.run(
    ...
    pockets ? 1 : 0, reshuffleVote, paymentPlan, comments, contactChangedAt || 0, contactChangeNote, Date.now()
  );
...
export function getActiveFineCategories() { return stmtActiveFineCategories.all(); }
export function getAllFineCategories()    { return stmtAllFineCategories.all(); }
export function getFineCategory(id)       { return db.prepare('SELECT * FROM fine_categories WHERE id = ?').get(id); }
const stmtInsertFineCategory = db.prepare(`
  INSERT INTO fine_categories (id, label, amount, examples, sort_order) VALUES (?, ?, ?, ?, ?)
`);
export function createFineCategory({ label, amount, examples = '' }) {
  ...
  stmtInsertFineCategory.run(id, label, amount, examples, maxOrder + 1);
  return id;
}
const stmtUpdateFineCategory = db.prepare(`UPDATE fine_categories SET label = ?, amount = ?, examples = ? WHERE id = ?`);
export function updateFineCategory(id, { label, amount, examples = '' }) { stmtUpdateFineCategory.run(label, amount, examples, id); }
...
const stmtInsertFineCase = db.prepare(`
  INSERT INTO fine_cases (id, player_id, player_name, game_id, category_id, category_label, amount, description, reported_by_type, reported_by_id, reported_by_name, created_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
export function createFineCase({ playerId, gameId = '', categoryId, description, reportedByType, reportedById, reportedByName }) {
  ...
  stmtInsertFineCase.run(id, playerId, player.name, gameId, categoryId, category.label, category.amount, description, reportedByType, reportedById, reportedByName, Date.now());
  return { ok: true, id };
}
...
// Both players just need to be confirmed registrants of the same season — no shared-game
// requirement (deliberately simplified; see conversation history for the earlier,
// dropped "must have shared a game" proposal).
const stmtPlayerConfirmedInSeason = db.prepare(`
  SELECT 1 FROM season_signups ss
  JOIN registrations r ON ss.reg_id = r.id
  WHERE r.player_id = ? AND ss.season = ? AND ss.status = 'confirmed'
  LIMIT 1
`);
export function isPlayerConfirmedForSeason(playerId, season) {
  return !!stmtPlayerConfirmedInSeason.get(playerId, String(season));
}
