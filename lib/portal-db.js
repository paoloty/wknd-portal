...
const stmtSetCompareCache = db.prepare(
  `INSERT OR REPLACE INTO compare_cache (pair_key, stats_key, writeup, created_at, view_count, last_viewed_at)
   VALUES (?, ?, ?, ?, 0, NULL)`
);
...
function compareStatsKey(tA, tB) {
  return `${tA?.games_played|0}_${tA?.pts|0}_${tA?.reb|0}_${tA?.ast|0}_${tB?.games_played|0}_${tB?.pts|0}_${tB?.reb|0}_${tB?.ast|0}`;
}

export function getCompareCache(idA, idB, tA, tB) {
  const pair = comparePairKey(idA, idB);
  const stats = compareStatsKey(tA, tB);
