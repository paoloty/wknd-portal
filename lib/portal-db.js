...
const stmtGetSeasonLatestWeek = db.prepare(
  `SELECT (COUNT(*) + 1) / 2 AS week FROM games WHERE season = ? AND game_type = 'regular' AND under_review = 0 AND date <= date('now')`
);
...
const stmtRecentPlayedGames = db.prepare(`
  SELECT id, date, team_a_name, team_b_name, team_a_score, team_b_score,
         game_type, season, under_review
  FROM games
  WHERE scheduled = 0 AND under_review = 0
  ORDER BY date DESC, id DESC
  LIMIT 5
`);
...
const stmtCountPlayedGames = db.prepare(
  `SELECT COUNT(*) AS n FROM games WHERE scheduled = 0 AND under_review = 0`
);
...
const stmtGetPlayersWithRatings = db.prepare(`
  SELECT p.id, p.name, p.number, p.status, p.sort_order,
         ...
         COALESCE((SELECT json_group_array(pp.position ORDER BY pp.sort_order)
                   FROM player_positions pp WHERE pp.player_id = p.id), '[]') AS positions,
         r.scoring, r.shooting, r.rebounding, r.playmaking, r.defense, r.iq, r.athleticism, r.overall,
         r.scoring_ovr, r.shooting_ovr, r.rebounding_ovr, r.playmaking_ovr,
         r.defense_ovr, r.iq_ovr, r.athleticism_ovr, r.overall_ovr,
         ...
         COALESCE(r.athleticism_ovr, r.athleticism) AS eff_athleticism
  FROM players p
  LEFT JOIN teams t ON t.id = p.team_id
...
const stmtUpsertComputedRating = db.prepare(`
  INSERT INTO player_ratings
    (player_id, season, scoring, shooting, rebounding, playmaking, defense, iq, athleticism, overall, computed_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ON CONFLICT(player_id, season) DO UPDATE SET
    ...
    athleticism=excluded.athleticism, overall=excluded.overall, computed_at=excluded.computed_at
  WHERE locked = 0
`);
...
const stmtSaveRatingOverrides = db.prepare(`
  INSERT INTO player_ratings
    (player_id, season, scoring_ovr, shooting_ovr, rebounding_ovr, playmaking_ovr,
     defense_ovr, iq_ovr, athleticism_ovr, overall_ovr, updated_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ON CONFLICT(player_id, season) DO UPDATE SET
    ...
    athleticism_ovr=excluded.athleticism_ovr, overall_ovr=excluded.overall_ovr,
...
export function saveRatingOverrides(playerId, season, ovr) {
  ...
  stmtSaveRatingOverrides.run(
    ...
    n(ovr.defense), n(ovr.iq), n(ovr.athleticism), n(ovr.overall),
    Date.now()
  );
}
...
const stmtGetStatsBySeason = db.prepare(`
  ...
  FROM game_player_stats gps
  JOIN games g ON g.id = gps.game_id
  WHERE CAST(g.season AS TEXT) = ? AND g.under_review = 0
...
const stmtGetOnePlayerStatsBySeason = db.prepare(`
  ...
  FROM game_player_stats gps
  JOIN games g ON g.id = gps.game_id
  WHERE gps.player_id = ? AND CAST(g.season AS TEXT) = ? AND g.under_review = 0
`);
...
const stmtPrevMatchup = db.prepare(`
  ...
  FROM games
  WHERE under_review = 0 AND id != ? AND scheduled = 0
