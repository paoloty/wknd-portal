...
const stmtInsertMarketplaceListing = db.prepare(`
  INSERT INTO marketplace_listings
    (id, type, seller_id, created_by_reg_id, title, description, price, photos, status,
     min_buyers, variant_options, created_at, updated_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
export function createMarketplaceListing({
  type, sellerId = '', createdByRegId = '', title, description = '', price = 0,
  photos = [], status = 'active', minBuyers = 0, variantOptions = [],
}) {
  const id = crypto.randomUUID();
  const now = Date.now();
  stmtInsertMarketplaceListing.run(
    id, type, sellerId, createdByRegId, title, description, Number(price) || 0,
...
const stmtUpdateMarketplaceListing = db.prepare(`
  ...
  SET title = ?, description = ?, price = ?, min_buyers = ?, variant_options = ?, updated_at = ?
  WHERE id = ?
`);
export function updateMarketplaceListing(id, { title, description = '', price = 0, minBuyers = 0, variantOptions = [] }) {
  stmtUpdateMarketplaceListing.run(title, description, Number(price) || 0, Number(minBuyers) || 0, JSON.stringify(variantOptions), Date.now(), id);
}
