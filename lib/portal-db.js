...
if (!_gamesCols.includes('status')) {
  ...
  db.exec(`UPDATE games SET status = 'final'     WHERE scheduled = 0 AND under_review = 1`);
  // existing under_review=0 scheduled=0 games keep default 'complete'
}
...
const stmtGetTeamRecords = db.prepare(`
  ...
  FROM (
    ...
    WHERE season = (SELECT MAX(season) FROM games WHERE game_type = 'regular' AND status IN ('final','complete'))
      AND game_type = 'regular' AND status IN ('final','complete')
    UNION ALL
