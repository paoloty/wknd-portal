...
const stmtGetSeasonAwards = db.prepare(`
  ...
  LEFT JOIN teams t ON t.id = p.team_id
  LEFT JOIN game_player_stats gps ON gps.player_id = a.player_id
  LEFT JOIN games g ON g.id = gps.game_id
    ...
    AND g.status = 'complete'
  WHERE a.season = ?
  GROUP BY a.id, a.award_type, a.season, a.notes, a.player_id, a.team_id,
           p.name, p.picture_url, t.name, t.color
  ORDER BY a.id ASC
`);
