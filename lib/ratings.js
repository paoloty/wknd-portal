...
export function computeRatings(stats, context = {}) {
  ...
  // IQ: AST/TOV ratio + TOV% penalty + foul penalty
  // TOV% = TOV / (FGA + 0.44×FTA + TOV) — per-game proxy
  const fga_pg = b_fg2m + b_fg3m + b_fg2m_miss + b_fg3m_miss;
  const fta_pg = b_ftm + b_ft_miss;
  const tov_rate = (fga_pg + 0.44 * fta_pg + tpg) > 0
    ? tpg / (fga_pg + 0.44 * fta_pg + tpg)
    : 0;
  const tov_rate_penalty = Math.max(0, (tov_rate - 0.15) * 100 * 1.5);

  let iq = 62;
  if (tpg > 0)      iq = clamp(52 + (apg / tpg) * 18, 52, 90);
  else if (apg > 0) iq = clamp(65 + apg * 3, 65, 90);
  iq = clamp(iq - tov_rate_penalty - Math.max(0, fpg - 3) * 4, 50, 92);
