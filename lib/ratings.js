...
function blendStats(season, recent) {
  ...
  return {
    ...
    _fg4m:      0.5 * (season.fg4m      || 0) / sgp + 0.5 * (recent.fg4m      || 0) / rgp,
    ...
    _fg4m_miss: 0.5 * (season.fg4m_miss || 0) / sgp + 0.5 * (recent.fg4m_miss || 0) / rgp,
...
export function computeRawValues(stats, context = {}) {
  ...
  const b_fg4m      = blended ? blended._fg4m      : (stats.fg4m      || 0) / gp;
  ...
  const b_fg4m_miss = blended ? blended._fg4m_miss : (stats.fg4m_miss || 0) / gp;
  const b_ftm       = blended ? blended._ftm       : (stats.ftm       || 0) / gp;
  const b_ft_miss   = blended ? blended._ft_miss   : (stats.ft_miss   || 0) / gp;

  const b_fga    = b_fg2m + b_fg3m + b_fg4m + b_fg2m_miss + b_fg3m_miss + b_fg4m_miss;
  ...
  if (teamTotals && (teamTotals.fga + teamTotals.fta + teamTotals.tov) > 0) {
    const p_fga = (stats.fg2m || 0) + (stats.fg3m || 0) + (stats.fg4m || 0) + (stats.fg2m_miss || 0) + (stats.fg3m_miss || 0) + (stats.fg4m_miss || 0);
