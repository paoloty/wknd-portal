...
// Quill leaves behind fully-empty <p> or <div> blocks (trailing cursor
// position, or leftovers from saves made before the editor had its own
// cleanup). Strip them before title/body splitting below — otherwise a
// leading empty block gets matched as the "title" and the real headline
// ends up in the body.
...
export function parseWriteup(writeup) {
  const s = stripEmptyParagraphs(String(writeup || '')).trim();
