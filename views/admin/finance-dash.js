...
<div class="${CARD} px-5 py-4 mb-5">
  <div class="flex items-center justify-between mb-1">
    ...
    <span class="text-xs text-slate-500">${fmt(quotaPaid)} of ${fmt(quota * quotaPlayerCount)} target (${quotaPlayerCount} × ${fmt(quota)})</span>
  </div>
  ${progressBar(quotaPaid, quota * quotaPlayerCount)}
</div>` : ''}` : '';
