...
export function scoreTicker(games) {
  const cards = games.map(g => {
    ...
    const top = winA
      ? { name: g.team_a_name, score: scoreA, color: teamColor(g.team_a_name), win: true }
      : { name: g.team_b_name, score: scoreB, color: teamColor(g.team_b_name), win: scoreB > scoreA };
    const bot = winA
      ? { name: g.team_b_name, score: scoreB, color: teamColor(g.team_b_name), win: false }
      : { name: g.team_a_name, score: scoreA, color: teamColor(g.team_a_name), win: false };
    const winnerColor = top.win ? top.color : 'var(--border)';

    const row = t => `<div class="ticker-team-row${t.win ? ' ticker-row--win' : ''}">
    <div class="ticker-team">
      <span class="team-dot" style="background:${t.color}"></span>
      <span class="ticker-team-name">${escHtml(t.name)}</span>
    </div>
